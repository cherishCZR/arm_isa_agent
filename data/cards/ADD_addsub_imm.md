## ADD
_ARM A64 Instruction_

**Title**: ADD (immediate) -- A64 | **Class**: `general` | **XML ID**: `ADD_addsub_imm`

**Summary**: Add immediate value

**Description**:
This instruction adds a register value and an optionally-shifted
immediate value, and writes the result to the destination register.

### Variant: `Not setting the condition flags (ADD_32_addsub_imm)` (32-bit)
- **Condition**: `sf == 0`
- **Assembly**: `ADD  <Wd|WSP>, <Wn|WSP>, #<imm>{, <shift>}`
- **Fixed bits**: `sf`=`0`
- **Bit Pattern**: `???????????????????????????????0`
**Encoding Diagram (32-bit)**:

```text
| 31 30 29 28  22 21   9   4  |
|--------------------------|
| sf  0   0   100010 sh  imm12 Rn  Rd  |
```

#### Decode (A64.dpimm.addsub_imm.ADD_32_addsub_imm)

```
constant integer d = UInt(Rd);
constant integer n = UInt(Rn);
constant integer datasize = 32 << UInt(sf);

constant bits(24) imm = if sh == '0' then Zeros(12):imm12 else imm12:Zeros(12);
```

#### Execute (A64.dpimm.addsub_imm.ADD_32_addsub_imm)

```
constant bits(datasize) operand1 = if n == 31 then SP[datasize] else X[n, datasize];
constant bits(datasize) operand2 = ZeroExtend(imm, datasize);
bits(datasize) result;
(result, -) = AddWithCarry(operand1, operand2, '0');

if d == 31 then
    SP[64] = ZeroExtend(result, 64);
else
    X[d, datasize] = result;
```

### Variant: `Not setting the condition flags (ADD_64_addsub_imm)` (64-bit)
- **Condition**: `sf == 1`
- **Assembly**: `ADD  <Xd|SP>, <Xn|SP>, #<imm>{, <shift>}`
- **Fixed bits**: `sf`=`1`
- **Bit Pattern**: `???????????????????????????????1`
**Encoding Diagram (32-bit)**:

```text
| 31 30 29 28  22 21   9   4  |
|--------------------------|
| sf  0   0   100010 sh  imm12 Rn  Rd  |
```

### Operands

| Symbol | Type | Field | Description |
|---|---|---|---|
| `<Wd\|WSP>` | `register (32-bit)` | `Rd` | Is the 32-bit name of the destination general-purpose register or stack pointer, encoded in the "Rd" field. |
| `<Wn\|WSP>` | `register (32-bit)` | `Rn` | Is the 32-bit name of the source general-purpose register or stack pointer, encoded in the "Rn" field. |
| `<imm>` | `immediate` | `imm12` | Is an unsigned immediate, in the range 0 to 4095, encoded in the "imm12" field. |
| `<shift>` | `shift` | `sh` | Is the optional left shift to apply to the immediate, defaulting to LSL #0 and |
| `<Xd\|SP>` | `register (64-bit)` | `Rd` | Is the 64-bit name of the destination general-purpose register or stack pointer, encoded in the "Rd" field. |
| `<Xn\|SP>` | `register (64-bit)` | `Rn` | Is the 64-bit name of the source general-purpose register or stack pointer, encoded in the "Rn" field. |

**<shift> Value Table**:

| bitfield | symbol |
|---|---|
| 0 | LSL #0 |
| 1 | LSL #12 |

### Operational Notes

If PSTATE.DIT is 1:
        
          
            The execution time of this instruction is independent of:
                
                  The values of the data supplied in any of its registers.
                
                
                  The values of the NZCV flags.
                
              
            
          
          
            The response of this instruction to asynchronous exceptions does not vary based on:
                
                  The values of the data supplied in any of its registers.
                
                
                  The values of the NZCV flags.

---
<details><summary>Metadata</summary>

- cond-setting: `no-s`
- immediate-type: `imm12u`
- isa: `A64`
- source: `add_addsub_imm.xml`
</details>