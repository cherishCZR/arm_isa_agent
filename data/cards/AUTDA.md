...
**Title**: AUTDA, AUTDZA -- A64 | **Class**: `general` | **XML ID**: `AUTDA`

**Architecture**: `FEAT_PAuth` (ARMv8.3)

**Summary**: Authenticate data address, using key A
...
### Variant: `Integer (AUTDA_64P_dp_1src)` (AUTDA)
- **Condition**: `Z == 0`
- **Assembly**: `AUTDA  <Xd>, <Xn|SP>`
- **Fixed bits**: `Z`=`0`
- **Bit Pattern**: `?????????????0??????????????????`
...
|--------------------------------|
| 1   1   0   11010110 00001 00  Z   110 Rn  Rd  |
```

#### Decode (A64.dpreg.dp_1src.AUTDA_64P_dp_1src)
...
#### Execute (A64.dpreg.dp_1src.AUTDA_64P_dp_1src)

```
if source_is_sp then
    X[d, 64] = AuthDA(X[d, 64], SP[64], auth_combined);
else
...
### Variant: `Integer (AUTDZA_64Z_dp_1src)` (AUTDZA)
