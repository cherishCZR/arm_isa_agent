...
**Title**: BFXIL -- A64 | **Class**: `general` | **XML ID**: `BFXIL_BFM`

**Summary**: Bitfield extract and insert at low end
...
### Variant: `Leaving other bits unchanged (BFXIL_BFM_32M_bitfield)` (32-bit)
- **Condition**: `sf == 0 && N == 0`
- **Assembly**: `BFXIL  <Wd>, <Wn>, #<lsb>, #<width>`
- **Fixed bits**: `sf`=`0`, `N`=`0`
- **Bit Pattern**: `??????????????????????0????????0`
- **Alias of**: `BFM  <Wd>, <Wn>, #<lsb>, #(<lsb>+<width>-1)`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  22 21  15   9   4  |
|--------------------------|
| sf  01  100110 N   immr imms Rn  Rd  |
```

### Variant: `Leaving other bits unchanged (BFXIL_BFM_64M_bitfield)` (64-bit)
- **Condition**: `sf == 1 && N == 1`
- **Assembly**: `BFXIL  <Xd>, <Xn>, #<lsb>, #<width>`
- **Fixed bits**: `sf`=`1`, `N`=`1`
- **Bit Pattern**: `??????????????????????1????????1`
- **Alias of**: `BFM  <Xd>, <Xn>, #<lsb>, #(<lsb>+<width>-1)`
**Encoding Diagram (32-bit)**:
...
- alias_mnemonic: `BFXIL`
- bitfield-fill: `nofill`
- isa: `A64`
- source: `bfxil_bfm.xml`
</details>