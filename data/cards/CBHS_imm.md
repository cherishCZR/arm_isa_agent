...
**Title**: CBHS (immediate) -- A64 | **Class**: `general` | **XML ID**: `CBHS_imm`

**Architecture**: `FEAT_CMPBR` (ARMv9.6)

**Summary**: Compare unsigned greater than or equal immediate and branch
...
### Variant: `Branch (CBHS_CBHI_32_imm)` (32-bit higher)
- **Condition**: `sf == 0`
- **Assembly**: `CBHS  <Wt>, #<immp1>, <label>`
- **Fixed bits**: `sf`=`0`
- **Bit Pattern**: `???????????????????????????????0`
- **Alias of**: `CBHI  <Wt>, #<imm>, <label>`
  Condition: Never
...
| 31 30  23  20  14 13   4  |
|-----------------------|
| sf  1110101 010 imm6 0   imm9 Rt  |
```

### Variant: `Branch (CBHS_CBHI_64_imm)` (64-bit higher)
- **Condition**: `sf == 1`
- **Assembly**: `CBHS  <Xt>, #<immp1>, <label>`
- **Fixed bits**: `sf`=`1`
- **Bit Pattern**: `???????????????????????????????1`
- **Alias of**: `CBHI  <Xt>, #<imm>, <label>`
  Condition: Never
**Encoding Diagram (32-bit)**:
