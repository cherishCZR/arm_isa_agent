...
**Title**: CBH<cc> -- A64 | **Class**: `general` | **XML ID**: `CBHcc_regs`

**Architecture**: `FEAT_CMPBR` (ARMv9.6)

**Summary**: Compare halfwords and branch

**Description**:
This instruction compares the halfword values in two registers, and conditionally
...
### Variant: `Branch (CBHGT_16_regs)` (Greater than)
- **Condition**: `cc == 000`
- **Assembly**: `CBHGT  <Wt>, <Wm>, <label>`
- **Fixed bits**: `cc`=`000`
- **Bit Pattern**: `?????????????????????000????????`
...
| 31  23  20  15 14 13   4  |
|-----------------------|
| 01110100 cc  Rm  1   1   imm9 Rt  |
```

#### Decode (A64.control.compbranch_regs2.CBHGT_16_regs)
...
#### Execute (A64.control.compbranch_regs2.CBHGT_16_regs)
...
### Variant: `Branch (CBHGE_16_regs)` (Greater than or equal)
- **Condition**: `cc == 001`
- **Assembly**: `CBHGE  <Wt>, <Wm>, <label>`
- **Fixed bits**: `cc`=`001`
- **Bit Pattern**: `?????????????????????100????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Branch (CBHHI_16_regs)` (Higher)
- **Condition**: `cc == 010`
- **Assembly**: `CBHHI  <Wt>, <Wm>, <label>`
- **Fixed bits**: `cc`=`010`
- **Bit Pattern**: `?????????????????????010????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Branch (CBHHS_16_regs)` (Higher or same)
- **Condition**: `cc == 011`
- **Assembly**: `CBHHS  <Wt>, <Wm>, <label>`
- **Fixed bits**: `cc`=`011`
- **Bit Pattern**: `?????????????????????110????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Branch (CBHEQ_16_regs)` (Equal)
- **Condition**: `cc == 110`
- **Assembly**: `CBHEQ  <Wt>, <Wm>, <label>`
- **Fixed bits**: `cc`=`110`
- **Bit Pattern**: `?????????????????????011????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Branch (CBHNE_16_regs)` (Not equal)
- **Condition**: `cc == 111`
- **Assembly**: `CBHNE  <Wt>, <Wm>, <label>`
- **Fixed bits**: `cc`=`111`
- **Bit Pattern**: `?????????????????????111????????`
**Encoding Diagram (32-bit)**:
