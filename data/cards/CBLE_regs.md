...
**Title**: CBLE (register) -- A64 | **Class**: `general` | **XML ID**: `CBLE_regs`

**Architecture**: `FEAT_CMPBR` (ARMv9.6)

**Summary**: Compare signed less than or equal register and branch
...
### Variant: `Branch (CBLE_CBGE_32_regs)` (32-bit greater than or equal)
- **Condition**: `sf == 0`
- **Assembly**: `CBLE  <Wm>, <Wt>, <label>`
- **Fixed bits**: `sf`=`0`
- **Bit Pattern**: `???????????????????????????????0`
- **Alias of**: `CBGE  <Wt>, <Wm>, <label>`
  Condition: Never
...
| 31 30  23  20  15  13   4  |
|-----------------------|
| sf  1110100 001 Rm  00  imm9 Rt  |
```

### Variant: `Branch (CBLE_CBGE_64_regs)` (64-bit greater than or equal)
- **Condition**: `sf == 1`
- **Assembly**: `CBLE  <Xm>, <Xt>, <label>`
- **Fixed bits**: `sf`=`1`
- **Bit Pattern**: `???????????????????????????????1`
- **Alias of**: `CBGE  <Xt>, <Xm>, <label>`
  Condition: Never
**Encoding Diagram (32-bit)**:
