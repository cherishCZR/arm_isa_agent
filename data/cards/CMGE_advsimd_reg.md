...
**Title**: CMGE (register) -- A64 | **Class**: `advsimd` | **XML ID**: `CMGE_advsimd_reg`

**Architecture**: `FEAT_AdvSIMD` (ARMv8.0)

**Summary**: Compare signed greater than or equal (vector)

**Description**:
This instruction compares each vector element in the
first source SIMD&FP register with the corresponding
vector element in the second source SIMD&FP register
and if the first signed integer value is greater than or equal to
the second signed integer value
...
### Variant: `Scalar`
- **Assembly**: `CMGE  D<d>, D<n>, D<m>`
**Encoding Diagram (32-bit)**:

```text
| 31  29 28 27  24 23  21 20  15  11 10  9   4  |
|-----------------------------------------|
| 01  0   1   111 0   11  1   Rm  0011 1   1   Rn  Rd  |
```

#### Decode (A64.simd_dp.asisdsame.CMGE_asisdsame_only)
...
#### Execute (A64.simd_dp.asisdsame.CMGE_asisdsame_only)
...
for e = 0 to elements-1
    ...
    Elem[result, e, esize] = if element1 >= element2 then Ones(esize) else Zeros(esize);
...
|--------------------------------------------|
| 0   Q   0   0   111 0   size 1   Rm  0011 1   1   Rn  Rd  |
```

#### Decode (A64.simd_dp.asimdsame.CMGE_asimdsame_only)
