...
**Title**: CMP (shifted register) -- A64 | **Class**: `general` | **XML ID**: `CMP_SUBS_addsub_shift`

**Summary**: Compare (shifted register)

**Description**:
This instruction subtracts an optionally-shifted register
value from a register value. It updates the condition flags based
on the result, and discards the result.

### Variant: `Setting the condition flags (CMP_SUBS_32_addsub_shift)` (32-bit)
- **Condition**: `sf == 0`
- **Assembly**: `CMP  <Wn>, <Wm>{, <shift> #<amount>}`
- **Fixed bits**: `sf`=`0`
- **Bit Pattern**: `???????????????????????????????0`
- **Alias of**: `SUBS  WZR, <Wn>, <Wm>{, <shift> #<amount>}`
...
|--------------------------------|
| sf  1   1   01011 shift 0   Rm  imm6 Rn  11111 |
```

### Variant: `Setting the condition flags (CMP_SUBS_64_addsub_shift)` (64-bit)
- **Condition**: `sf == 1`
- **Assembly**: `CMP  <Xn>, <Xm>{, <shift> #<amount>}`
- **Fixed bits**: `sf`=`1`
- **Bit Pattern**: `???????????????????????????????1`
- **Alias of**: `SUBS  XZR, <Xn>, <Xm>{, <shift> #<amount>}`
  Condition: Unconditionally
**Encoding Diagram (32-bit)**:
...
- cond-setting: `S`
- isa: `A64`
- source: `cmp_subs_addsub_shift.xml`
</details>