...
**Title**: CPYFPN, CPYFMN, CPYFEN -- A64 | **Class**: `general` | **XML ID**: `CPYFPN`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy forward-only, reads and writes non-temporal
...
CPYFMN instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYFMN copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYFEN copies any final remaining bytes.
...
### Variant: `Integer (CPYFPN_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYFPN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 0   01  op1 0   Rs  1100 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYFPN_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYFPN_CPY_memcms)
...
### Variant: `Integer (CPYFMN_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYFMN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYFEN_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYFEN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
