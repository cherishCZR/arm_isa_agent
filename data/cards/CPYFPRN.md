...
**Title**: CPYFPRN, CPYFMRN, CPYFERN -- A64 | **Class**: `general` | **XML ID**: `CPYFPRN`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy forward-only, reads non-temporal
...
CPYFPRN, then CPYFMRN, and then CPYFERN.

CPYFPRN performs some preconditioning of the arguments suitable for using the
CPYFMRN instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYFMRN copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYFERN copies any final remaining bytes.
...
### Variant: `Integer (CPYFPRN_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYFPRN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 0   01  op1 0   Rs  1000 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYFPRN_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYFPRN_CPY_memcms)
...
### Variant: `Integer (CPYFMRN_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYFMRN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYFERN_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYFERN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
