...
**Title**: CPYFPRT, CPYFMRT, CPYFERT -- A64 | **Class**: `general` | **XML ID**: `CPYFPRT`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy forward-only, reads unprivileged
...
CPYFPRT, then CPYFMRT, and then CPYFERT.

CPYFPRT performs some preconditioning of the arguments suitable for using the
CPYFMRT instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYFMRT copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYFERT copies any final remaining bytes.
...
### Variant: `Integer (CPYFPRT_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYFPRT  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 0   01  op1 0   Rs  0010 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYFPRT_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYFPRT_CPY_memcms)
...
### Variant: `Integer (CPYFMRT_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYFMRT  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYFERT_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYFERT  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
