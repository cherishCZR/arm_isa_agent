...
**Title**: CPYFPRTWN, CPYFMRTWN, CPYFERTWN -- A64 | **Class**: `general` | **XML ID**: `CPYFPRTWN`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy forward-only, reads unprivileged, writes non-temporal
...
CPYFPRTWN, then CPYFMRTWN, and then CPYFERTWN.

CPYFPRTWN performs some preconditioning of the arguments suitable for using the
CPYFMRTWN instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYFMRTWN copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYFERTWN copies any final remaining bytes.
...
### Variant: `Integer (CPYFPRTWN_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYFPRTWN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 0   01  op1 0   Rs  0110 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYFPRTWN_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYFPRTWN_CPY_memcms)
...
### Variant: `Integer (CPYFMRTWN_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYFMRTWN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYFERTWN_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYFERTWN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
