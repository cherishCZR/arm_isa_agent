...
**Title**: CPYFPT, CPYFMT, CPYFET -- A64 | **Class**: `general` | **XML ID**: `CPYFPT`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy forward-only, reads and writes unprivileged
...
CPYFMT instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYFMT copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYFET copies any final remaining bytes.
...
### Variant: `Integer (CPYFPT_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYFPT  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 0   01  op1 0   Rs  0011 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYFPT_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYFPT_CPY_memcms)
...
### Variant: `Integer (CPYFMT_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYFMT  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYFET_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYFET  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
