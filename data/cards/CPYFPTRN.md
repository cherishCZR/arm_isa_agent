...
**Title**: CPYFPTRN, CPYFMTRN, CPYFETRN -- A64 | **Class**: `general` | **XML ID**: `CPYFPTRN`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy forward-only, reads and writes unprivileged, reads non-temporal
...
CPYFPTRN, then CPYFMTRN, and then CPYFETRN.

CPYFPTRN performs some preconditioning of the arguments suitable for using the
CPYFMTRN instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYFMTRN copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYFETRN copies any final remaining bytes.
...
### Variant: `Integer (CPYFPTRN_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYFPTRN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 0   01  op1 0   Rs  1011 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYFPTRN_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYFPTRN_CPY_memcms)
...
### Variant: `Integer (CPYFMTRN_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYFMTRN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYFETRN_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYFETRN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
