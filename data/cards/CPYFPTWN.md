...
**Title**: CPYFPTWN, CPYFMTWN, CPYFETWN -- A64 | **Class**: `general` | **XML ID**: `CPYFPTWN`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy forward-only, reads and writes unprivileged, writes non-temporal
...
CPYFPTWN, then CPYFMTWN, and then CPYFETWN.

CPYFPTWN performs some preconditioning of the arguments suitable for using the
CPYFMTWN instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYFMTWN copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYFETWN copies any final remaining bytes.
...
### Variant: `Integer (CPYFPTWN_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYFPTWN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 0   01  op1 0   Rs  0111 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYFPTWN_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYFPTWN_CPY_memcms)
...
### Variant: `Integer (CPYFMTWN_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYFMTWN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYFETWN_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYFETWN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
