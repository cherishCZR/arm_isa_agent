...
**Title**: CPYFPWT, CPYFMWT, CPYFEWT -- A64 | **Class**: `general` | **XML ID**: `CPYFPWT`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy forward-only, writes unprivileged
...
CPYFPWT, then CPYFMWT, and then CPYFEWT.

CPYFPWT performs some preconditioning of the arguments suitable for using the
CPYFMWT instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYFMWT copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYFEWT copies any final remaining bytes.
...
### Variant: `Integer (CPYFPWT_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYFPWT  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 0   01  op1 0   Rs  0001 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYFPWT_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYFPWT_CPY_memcms)
...
### Variant: `Integer (CPYFMWT_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYFMWT  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYFEWT_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYFEWT  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
