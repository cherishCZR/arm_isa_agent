...
**Title**: CPYFPWTN, CPYFMWTN, CPYFEWTN -- A64 | **Class**: `general` | **XML ID**: `CPYFPWTN`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy forward-only, writes unprivileged, reads and writes non-temporal
...
CPYFPWTN, then CPYFMWTN, and then CPYFEWTN.

CPYFPWTN performs some preconditioning of the arguments suitable for using the
CPYFMWTN instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYFMWTN copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYFEWTN copies any final remaining bytes.
...
### Variant: `Integer (CPYFPWTN_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYFPWTN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 0   01  op1 0   Rs  1101 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYFPWTN_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYFPWTN_CPY_memcms)
...
### Variant: `Integer (CPYFMWTN_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYFMWTN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYFEWTN_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYFEWTN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
