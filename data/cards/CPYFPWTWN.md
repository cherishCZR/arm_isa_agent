...
**Title**: CPYFPWTWN, CPYFMWTWN, CPYFEWTWN -- A64 | **Class**: `general` | **XML ID**: `CPYFPWTWN`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy forward-only, writes unprivileged and non-temporal
...
CPYFPWTWN, then CPYFMWTWN, and then CPYFEWTWN.

CPYFPWTWN performs some preconditioning of the arguments suitable for using the
CPYFMWTWN instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYFMWTWN copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYFEWTWN copies any final remaining bytes.
...
### Variant: `Integer (CPYFPWTWN_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYFPWTWN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 0   01  op1 0   Rs  0101 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYFPWTWN_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYFPWTWN_CPY_memcms)
...
### Variant: `Integer (CPYFMWTWN_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYFMWTWN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYFEWTWN_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYFEWTWN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
