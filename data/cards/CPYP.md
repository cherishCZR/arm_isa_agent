...
**Title**: CPYP, CPYM, CPYE -- A64 | **Class**: `general` | **XML ID**: `CPYP`
...
in succession and to appear consecutively in memory: CPYP, then CPYM,
and then CPYE.

CPYP performs some preconditioning of the arguments suitable for using the
CPYM instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYM copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYE copies any final remaining bytes.
...
### Variant: `Integer (CPYP_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYP  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 1   01  op1 0   Rs  0000 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYP_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYP_CPY_memcms)
...
### Variant: `Integer (CPYM_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYM  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYE_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYE  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
