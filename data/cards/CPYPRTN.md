...
**Title**: CPYPRTN, CPYMRTN, CPYERTN -- A64 | **Class**: `general` | **XML ID**: `CPYPRTN`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy, reads unprivileged, reads and writes non-temporal
...
in succession and to appear consecutively in memory: CPYPRTN, then CPYMRTN,
and then CPYERTN.

CPYPRTN performs some preconditioning of the arguments suitable for using the
CPYMRTN instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYMRTN copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYERTN copies any final remaining bytes.
...
### Variant: `Integer (CPYPRTN_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYPRTN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 1   01  op1 0   Rs  1110 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYPRTN_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYPRTN_CPY_memcms)
...
### Variant: `Integer (CPYMRTN_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYMRTN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYERTN_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYERTN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
