...
**Title**: CPYPRTRN, CPYMRTRN, CPYERTRN -- A64 | **Class**: `general` | **XML ID**: `CPYPRTRN`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy, reads unprivileged and non-temporal
...
in succession and to appear consecutively in memory: CPYPRTRN, then CPYMRTRN,
and then CPYERTRN.

CPYPRTRN performs some preconditioning of the arguments suitable for using the
CPYMRTRN instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYMRTRN copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYERTRN copies any final remaining bytes.
...
### Variant: `Integer (CPYPRTRN_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYPRTRN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 1   01  op1 0   Rs  1010 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYPRTRN_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYPRTRN_CPY_memcms)
...
### Variant: `Integer (CPYMRTRN_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYMRTRN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYERTRN_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYERTRN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
