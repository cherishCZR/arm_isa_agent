...
**Title**: CPYPRTWN, CPYMRTWN, CPYERTWN -- A64 | **Class**: `general` | **XML ID**: `CPYPRTWN`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy, reads unprivileged, writes non-temporal
...
in succession and to appear consecutively in memory: CPYPRTWN, then CPYMRTWN,
and then CPYERTWN.

CPYPRTWN performs some preconditioning of the arguments suitable for using the
CPYMRTWN instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYMRTWN copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYERTWN copies any final remaining bytes.
...
### Variant: `Integer (CPYPRTWN_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYPRTWN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 1   01  op1 0   Rs  0110 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYPRTWN_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYPRTWN_CPY_memcms)
...
### Variant: `Integer (CPYMRTWN_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYMRTWN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYERTWN_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYERTWN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
