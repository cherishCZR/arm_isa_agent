...
**Title**: CPYPT, CPYMT, CPYET -- A64 | **Class**: `general` | **XML ID**: `CPYPT`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy, reads and writes unprivileged
...
in succession and to appear consecutively in memory: CPYPT, then CPYMT,
and then CPYET.

CPYPT performs some preconditioning of the arguments suitable for using the
CPYMT instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYMT copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYET copies any final remaining bytes.
...
### Variant: `Integer (CPYPT_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYPT  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 1   01  op1 0   Rs  0011 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYPT_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYPT_CPY_memcms)
...
### Variant: `Integer (CPYMT_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYMT  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYET_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYET  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
