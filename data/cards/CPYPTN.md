...
**Title**: CPYPTN, CPYMTN, CPYETN -- A64 | **Class**: `general` | **XML ID**: `CPYPTN`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy, reads and writes unprivileged and non-temporal
...
in succession and to appear consecutively in memory: CPYPTN, then CPYMTN,
and then CPYETN.

CPYPTN performs some preconditioning of the arguments suitable for using the
CPYMTN instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYMTN copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYETN copies any final remaining bytes.
...
### Variant: `Integer (CPYPTN_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYPTN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 1   01  op1 0   Rs  1111 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYPTN_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYPTN_CPY_memcms)
...
### Variant: `Integer (CPYMTN_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYMTN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYETN_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYETN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
