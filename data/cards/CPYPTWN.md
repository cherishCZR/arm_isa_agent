...
**Title**: CPYPTWN, CPYMTWN, CPYETWN -- A64 | **Class**: `general` | **XML ID**: `CPYPTWN`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy, reads and writes unprivileged, writes non-temporal
...
in succession and to appear consecutively in memory: CPYPTWN, then CPYMTWN,
and then CPYETWN.

CPYPTWN performs some preconditioning of the arguments suitable for using the
CPYMTWN instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYMTWN copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYETWN copies any final remaining bytes.
...
### Variant: `Integer (CPYPTWN_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYPTWN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 1   01  op1 0   Rs  0111 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYPTWN_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYPTWN_CPY_memcms)
...
### Variant: `Integer (CPYMTWN_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYMTWN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYETWN_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYETWN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
