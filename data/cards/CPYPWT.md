...
**Title**: CPYPWT, CPYMWT, CPYEWT -- A64 | **Class**: `general` | **XML ID**: `CPYPWT`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy, writes unprivileged
...
in succession and to appear consecutively in memory: CPYPWT, then CPYMWT,
and then CPYEWT.

CPYPWT performs some preconditioning of the arguments suitable for using the
CPYMWT instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYMWT copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYEWT copies any final remaining bytes.
...
### Variant: `Integer (CPYPWT_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYPWT  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 1   01  op1 0   Rs  0001 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYPWT_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYPWT_CPY_memcms)
...
### Variant: `Integer (CPYMWT_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYMWT  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYEWT_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYEWT  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
