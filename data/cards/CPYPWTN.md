...
**Title**: CPYPWTN, CPYMWTN, CPYEWTN -- A64 | **Class**: `general` | **XML ID**: `CPYPWTN`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory copy, writes unprivileged, reads and writes non-temporal
...
CPYMWTN instruction, and copies an IMPLEMENTATION DEFINED portion of the requested
number of bytes. CPYMWTN copies a further IMPLEMENTATION DEFINED portion of the
remaining bytes. CPYEWTN copies any final remaining bytes.
...
### Variant: `Integer (CPYPWTN_CPY_memcms)` (Prologue)
- **Condition**: `op1 == 00`
- **Assembly**: `CPYPWTN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------|
| sz  011 1   01  op1 0   Rs  1101 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.CPYPWTN_CPY_memcms)
...
#### Execute (A64.ldst.memcms.CPYPWTN_CPY_memcms)
...
### Variant: `Integer (CPYMWTN_CPY_memcms)` (Main)
- **Condition**: `op1 == 01`
- **Assembly**: `CPYMWTN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`01`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (CPYEWTN_CPY_memcms)` (Epilogue)
- **Condition**: `op1 == 10`
- **Assembly**: `CPYEWTN  [<Xd>]!, [<Xs>]!, <Xn>!`
- **Fixed bits**: `op1`=`10`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
