...
**Title**: FCVTPU (scalar SIMD&FP) -- A64 | **Class**: `float` | **XML ID**: `FCVTPU_sisd`

**Architecture**: `FEAT_FPRCVT` (ARMv9.6)

**Summary**: Floating-point convert to unsigned integer, rounding toward plus infinity (scalar SIMD&FP)
...
the Round toward Plus Infinity rounding mode, and writes the result to the
...
### Variant: `Floating-point (FCVTPU_sisd_32H)` (Half-precision to 32-bit)
- **Condition**: `sf == 0 && ftype == 11`
- **Assembly**: `FCVTPU  <Sd>, <Hn>`
...
|-----------------------------------|
| sf  0   0   11110 ftype 1   10  011 000000 Rn  Rd  |
```

#### Decode (A64.simd_dp.float2int.FCVTPU_sisd_32H)
...
#### Execute (A64.simd_dp.float2int.FCVTPU_sisd_32H)
...
fltval = V[n, fltsize];
intval = FPToFixed(fltval, 0, TRUE, FPCR, FPRounding_POSINF, intsize);
...
### Variant: `Floating-point (FCVTPU_sisd_64H)` (Half-precision to 64-bit)
- **Condition**: `sf == 1 && ftype == 11`
- **Assembly**: `FCVTPU  <Dd>, <Hn>`
...
### Variant: `Floating-point (FCVTPU_sisd_64S)` (Single-precision to 64-bit)
- **Condition**: `sf == 1 && ftype == 00`
- **Assembly**: `FCVTPU  <Dd>, <Sn>`
...
### Variant: `Floating-point (FCVTPU_sisd_32D)` (Double-precision to 32-bit)
- **Condition**: `sf == 0 && ftype == 01`
- **Assembly**: `FCVTPU  <Sd>, <Dn>`
