...
**Title**: FDIV (scalar) -- A64 | **Class**: `float` | **XML ID**: `FDIV_float`

**Summary**: Floating-point divide (scalar)

**Description**:
This instruction divides the
floating-point value of the first source SIMD&FP register
by the floating-point value of the second source SIMD&FP register,
...
### Variant: `Floating-point (FDIV_H_floatdp2)` (Half-precision)
- **Condition**: `ftype == 11`
- **Assembly**: `FDIV  <Hd>, <Hn>, <Hm>`
- **Fixed bits**: `ftype`=`11`
- **Bit Pattern**: `??????????????????????11????????`
...
#### Decode (A64.simd_dp.floatdp2.FDIV_H_floatdp2)
...
#### Execute (A64.simd_dp.floatdp2.FDIV_H_floatdp2)
...
### Variant: `Floating-point (FDIV_S_floatdp2)` (Single-precision)
- **Condition**: `ftype == 00`
...
### Variant: `Floating-point (FDIV_D_floatdp2)` (Double-precision)
- **Condition**: `ftype == 01`
