...
**Title**: FMADD -- A64 | **Class**: `float` | **XML ID**: `FMADD_float`

**Summary**: Floating-point fused multiply-add (scalar)

**Description**:
This instruction multiplies the values of
the first two SIMD&FP source
registers, adds the product to the value of the third
SIMD&FP source register, and
...
### Variant: `Floating-point (FMADD_H_floatdp3)` (Half-precision)
- **Condition**: `ftype == 11`
- **Assembly**: `FMADD  <Hd>, <Hn>, <Hm>, <Ha>`
- **Fixed bits**: `ftype`=`11`
- **Bit Pattern**: `??????????????????????11????????`
...
|-----------------------------------|
| 0   0   0   11111 ftype 0   Rm  0   Ra  Rn  Rd  |
```

#### Decode (A64.simd_dp.floatdp3.FMADD_H_floatdp3)
...
#### Execute (A64.simd_dp.floatdp3.FMADD_H_floatdp3)
...
### Variant: `Floating-point (FMADD_S_floatdp3)` (Single-precision)
- **Condition**: `ftype == 00`
...
### Variant: `Floating-point (FMADD_D_floatdp3)` (Double-precision)
- **Condition**: `ftype == 01`
