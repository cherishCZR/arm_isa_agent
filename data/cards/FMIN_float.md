...
**Title**: FMIN (scalar) -- A64 | **Class**: `float` | **XML ID**: `FMIN_float`

**Summary**: Floating-point minimum (scalar)

**Description**:
This instruction compares the first and second
source SIMD&FP register values,
and writes the smaller of the two floating-point values to the
...
### Variant: `Floating-point (FMIN_H_floatdp2)` (Half-precision)
- **Condition**: `ftype == 11`
- **Assembly**: `FMIN  <Hd>, <Hn>, <Hm>`
- **Fixed bits**: `ftype`=`11`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:

```text
| 31 30 29 28  23  21 20  15  13  11   9   4  |
|--------------------------------------|
| 0   0   0   11110 ftype 1   Rm  01  01  10  Rn  Rd  |
```

#### Decode (A64.simd_dp.floatdp2.FMIN_H_floatdp2)
...
#### Execute (A64.simd_dp.floatdp2.FMIN_H_floatdp2)
...
### Variant: `Floating-point (FMIN_S_floatdp2)` (Single-precision)
- **Condition**: `ftype == 00`
...
### Variant: `Floating-point (FMIN_D_floatdp2)` (Double-precision)
- **Condition**: `ftype == 01`
