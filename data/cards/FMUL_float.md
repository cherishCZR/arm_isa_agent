...
**Title**: FMUL (scalar) -- A64 | **Class**: `float` | **XML ID**: `FMUL_float`

**Summary**: Floating-point multiply (scalar)

**Description**:
This instruction multiplies the floating-point values of the two source SIMD&FP
...
### Variant: `Floating-point (FMUL_H_floatdp2)` (Half-precision)
- **Condition**: `ftype == 11`
- **Assembly**: `FMUL  <Hd>, <Hn>, <Hm>`
- **Fixed bits**: `ftype`=`11`
- **Bit Pattern**: `??????????????????????11????????`
...
|-----------------------------------|
| 0   0   0   11110 ftype 1   Rm  0   00010 Rn  Rd  |
```

#### Decode (A64.simd_dp.floatdp2.FMUL_H_floatdp2)
...
#### Execute (A64.simd_dp.floatdp2.FMUL_H_floatdp2)
...
Elem[result, 0, esize]  = FPMul(operand1, operand2, FPCR);
...
### Variant: `Floating-point (FMUL_S_floatdp2)` (Single-precision)
- **Condition**: `ftype == 00`
...
### Variant: `Floating-point (FMUL_D_floatdp2)` (Double-precision)
- **Condition**: `ftype == 01`
