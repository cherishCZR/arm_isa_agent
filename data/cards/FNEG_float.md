...
**Title**: FNEG (scalar) -- A64 | **Class**: `float` | **XML ID**: `FNEG_float`

**Summary**: Floating-point negate (scalar)
...
### Variant: `Floating-point (FNEG_H_floatdp1)` (Half-precision)
- **Condition**: `ftype == 11`
- **Assembly**: `FNEG  <Hd>, <Hn>`
- **Fixed bits**: `ftype`=`11`
- **Bit Pattern**: `??????????????????????11????????`
...
|--------------------------------|
| 0   0   0   11110 ftype 10000 10  10000 Rn  Rd  |
```

#### Decode (A64.simd_dp.floatdp1.FNEG_H_floatdp1)
...
#### Execute (A64.simd_dp.floatdp1.FNEG_H_floatdp1)
...
### Variant: `Floating-point (FNEG_S_floatdp1)` (Single-precision)
- **Condition**: `ftype == 00`
- **Assembly**: `FNEG  <Sd>, <Sn>`
- **Fixed bits**: `ftype`=`00`
- **Bit Pattern**: `??????????????????????00????????`
...
### Variant: `Floating-point (FNEG_D_floatdp1)` (Double-precision)
- **Condition**: `ftype == 01`
- **Assembly**: `FNEG  <Dd>, <Dn>`
- **Fixed bits**: `ftype`=`01`
- **Bit Pattern**: `??????????????????????10????????`
