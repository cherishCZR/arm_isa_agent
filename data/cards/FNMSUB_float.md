...
**Title**: FNMSUB -- A64 | **Class**: `float` | **XML ID**: `FNMSUB_float`

**Summary**: Floating-point negated fused multiply-subtract (scalar)

**Description**:
This instruction multiplies the values of the first two
SIMD&FP source registers, subtracts the value of the third
SIMD&FP source register, and writes the result to the destination SIMD&FP register.
...
### Variant: `Floating-point (FNMSUB_H_floatdp3)` (Half-precision)
- **Condition**: `ftype == 11`
- **Assembly**: `FNMSUB  <Hd>, <Hn>, <Hm>, <Ha>`
- **Fixed bits**: `ftype`=`11`
- **Bit Pattern**: `??????????????????????11????????`
...
|-----------------------------------|
| 0   0   0   11111 ftype 1   Rm  1   Ra  Rn  Rd  |
```

#### Decode (A64.simd_dp.floatdp3.FNMSUB_H_floatdp3)
...
#### Execute (A64.simd_dp.floatdp3.FNMSUB_H_floatdp3)
...
### Variant: `Floating-point (FNMSUB_S_floatdp3)` (Single-precision)
- **Condition**: `ftype == 00`
...
### Variant: `Floating-point (FNMSUB_D_floatdp3)` (Double-precision)
- **Condition**: `ftype == 01`
