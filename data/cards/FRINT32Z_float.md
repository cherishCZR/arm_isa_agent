...
**Title**: FRINT32Z (scalar) -- A64 | **Class**: `float` | **XML ID**: `FRINT32Z_float`

**Architecture**: `FEAT_FRINTTS` (ARMv8.5)

**Summary**: Floating-point round to 32-bit integer toward zero (scalar)
...
### Variant: `Floating-point (FRINT32Z_S_floatdp1)` (Single-precision)
- **Condition**: `ftype == 00`
- **Assembly**: `FRINT32Z  <Sd>, <Sn>`
- **Fixed bits**: `ftype`=`0`
- **Bit Pattern**: `??????????????????????0?????????`
...
|--------------------------------|
| 0   0   0   11110 0x  10100 00  10000 Rn  Rd  |
```

#### Decode (A64.simd_dp.floatdp1.FRINT32Z_S_floatdp1)
...
#### Execute (A64.simd_dp.floatdp1.FRINT32Z_S_floatdp1)
...
### Variant: `Floating-point (FRINT32Z_D_floatdp1)` (Double-precision)
- **Condition**: `ftype == 01`
- **Assembly**: `FRINT32Z  <Dd>, <Dn>`
- **Fixed bits**: `ftype`=`1`
- **Bit Pattern**: `??????????????????????1?????????`
**Encoding Diagram (32-bit)**:
