...
**Title**: FRINTX (scalar) -- A64 | **Class**: `float` | **XML ID**: `FRINTX_float`

**Summary**: Floating-point round to integral exact, using current rounding mode (scalar)
...
rounding mode that is determined by the FPCR, and writes the
result to the SIMD&FP destination register.

When the result value is not numerically equal to
the input value, an Inexact exception is raised.
A zero input
...
### Variant: `Floating-point (FRINTX_H_floatdp1)` (Half-precision)
- **Condition**: `ftype == 11`
- **Assembly**: `FRINTX  <Hd>, <Hn>`
- **Fixed bits**: `ftype`=`11`
- **Bit Pattern**: `??????????????????????11????????`
...
|--------------------------------|
| 0   0   0   11110 ftype 1001 110 10000 Rn  Rd  |
```

#### Decode (A64.simd_dp.floatdp1.FRINTX_H_floatdp1)
...
#### Execute (A64.simd_dp.floatdp1.FRINTX_H_floatdp1)
...
### Variant: `Floating-point (FRINTX_S_floatdp1)` (Single-precision)
- **Condition**: `ftype == 00`
...
### Variant: `Floating-point (FRINTX_D_floatdp1)` (Double-precision)
- **Condition**: `ftype == 01`
