...
**Title**: FSUB (scalar) -- A64 | **Class**: `float` | **XML ID**: `FSUB_float`

**Summary**: Floating-point subtract (scalar)

**Description**:
This instruction subtracts the floating-point value
of the second source SIMD&FP register
from the floating-point value of the first source SIMD&FP register,
...
### Variant: `Floating-point (FSUB_H_floatdp2)` (Half-precision)
- **Condition**: `ftype == 11`
- **Assembly**: `FSUB  <Hd>, <Hn>, <Hm>`
- **Fixed bits**: `ftype`=`11`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:

```text
| 31 30 29 28  23  21 20  15  12 11   9   4  |
|--------------------------------------|
| 0   0   0   11110 ftype 1   Rm  001 1   10  Rn  Rd  |
```

#### Decode (A64.simd_dp.floatdp2.FSUB_H_floatdp2)
...
#### Execute (A64.simd_dp.floatdp2.FSUB_H_floatdp2)
...
### Variant: `Floating-point (FSUB_S_floatdp2)` (Single-precision)
- **Condition**: `ftype == 00`
...
### Variant: `Floating-point (FSUB_D_floatdp2)` (Double-precision)
- **Condition**: `ftype == 01`
