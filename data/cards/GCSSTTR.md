...
**Title**: GCSSTTR -- A64 | **Class**: `general` | **XML ID**: `GCSSTTR`

**Architecture**: `FEAT_GCS` (ARMv9.4)

**Summary**: Guarded Control Stack store register (unprivileged)

**Description**:
This instruction stores a
doubleword from a register to memory. The address that
is used for the store is calculated from a base register.
...
### Variant: `Integer`
- **Assembly**: `GCSSTTR  <Xt>, [<Xn|SP>]`
**Encoding Diagram (32-bit)**:

```text
| 31  27 26 25 24  14  11   9   4  |
|-----------------------------|
| 1101 1   0   0   1000111110 001 11  Rn  Rt  |
```

#### Decode (A64.ldst.ldst_gcs.GCSSTTR_64_ldst_gcs)
...
#### Execute (A64.ldst.ldst_gcs.GCSSTTR_64_ldst_gcs)

```
bits(64) address;

constant bits(2) effective_el = if AArch64.IsUnprivAccessPriv() then PSTATE.EL else EL0;
