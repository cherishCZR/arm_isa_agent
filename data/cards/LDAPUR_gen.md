...
**Title**: LDAPUR -- A64 | **Class**: `general` | **XML ID**: `LDAPUR_gen`

**Architecture**: `FEAT_LRCPC2` (ARMv8.4)

**Summary**: Load-acquire RCpc register (unscaled)
...
### Variant: `Unscaled offset (LDAPUR_32_ldapstl_unscaled)` (32-bit)
- **Condition**: `size == 10`
- **Assembly**: `LDAPUR  <Wt>, [<Xn|SP>{, #<simm>}]`
- **Fixed bits**: `size`=`0`
- **Bit Pattern**: `??????????????????????????????0?`
...
|--------------------------|
| 1x  011001 01  0   imm9 00  Rn  Rt  |
```

#### Decode (A64.ldst.ldapstl_unscaled.LDAPUR_32_ldapstl_unscaled)
...
#### Postdecode (A64.ldst.ldapstl_unscaled.LDAPUR_32_ldapstl_unscaled)
...
#### Execute (A64.ldst.ldapstl_unscaled.LDAPUR_32_ldapstl_unscaled)
...
### Variant: `Unscaled offset (LDAPUR_64_ldapstl_unscaled)` (64-bit)
- **Condition**: `size == 11`
- **Assembly**: `LDAPUR  <Xt>, [<Xn|SP>{, #<simm>}]`
- **Fixed bits**: `size`=`1`
- **Bit Pattern**: `??????????????????????????????1?`
**Encoding Diagram (32-bit)**:
