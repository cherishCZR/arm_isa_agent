...
**Title**: LDBFADD, LDBFADDA, LDBFADDAL, LDBFADDL -- A64 | **Class**: `advsimd` | **XML ID**: `LDBFADD`

**Architecture**: `FEAT_LSFE` (ARMv9.6)

**Summary**: BFloat16 floating-point add in memory
...
### Variant: `Floating-point (LDBFADD_16)` (No memory ordering)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `LDBFADD  <Hs>, <Ht>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------------|
| 00  111 1   00  A   R   1   Rs  0   000 00  Rn  Rt  |
```

#### Decode (A64.ldst.memop.LDBFADD_16)
...
#### Execute (A64.ldst.memop.LDBFADD_16)
...
bits(datasize) value;
bits(datasize) data;
constant AccessDescriptor accdesc = CreateAccDescFPAtomicOp(MemAtomicOp_BFADD, acquire,
                                                            release, tagchecked);
...
### Variant: `Floating-point (LDBFADDA_16)` (Acquire)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `LDBFADDA  <Hs>, <Ht>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Floating-point (LDBFADDAL_16)` (Acquire-release)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `LDBFADDAL  <Hs>, <Ht>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Floating-point (LDBFADDL_16)` (Release)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `LDBFADDL  <Hs>, <Ht>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
