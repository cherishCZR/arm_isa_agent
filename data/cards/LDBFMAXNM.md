...
**Title**: LDBFMAXNM, LDBFMAXNMA, LDBFMAXNMAL, LDBFMAXNML -- A64 | **Class**: `advsimd` | **XML ID**: `LDBFMAXNM`

**Architecture**: `FEAT_LSFE` (ARMv9.6)

**Summary**: BFloat16 floating-point atomic maximum number in memory
...
### Variant: `Floating-point (LDBFMAXNM_16)` (No memory ordering)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `LDBFMAXNM  <Hs>, <Ht>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------------|
| 00  111 1   00  A   R   1   Rs  0   110 00  Rn  Rt  |
```

#### Decode (A64.ldst.memop.LDBFMAXNM_16)
...
#### Execute (A64.ldst.memop.LDBFMAXNM_16)
...
bits(datasize) value;
bits(datasize) data;
constant AccessDescriptor accdesc = CreateAccDescFPAtomicOp(MemAtomicOp_BFMAXNM, acquire,
                                                            release, tagchecked);
...
### Variant: `Floating-point (LDBFMAXNMA_16)` (Acquire)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `LDBFMAXNMA  <Hs>, <Ht>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Floating-point (LDBFMAXNMAL_16)` (Acquire-release)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `LDBFMAXNMAL  <Hs>, <Ht>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Floating-point (LDBFMAXNML_16)` (Release)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `LDBFMAXNML  <Hs>, <Ht>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
