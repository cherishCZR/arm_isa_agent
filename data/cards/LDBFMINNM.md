...
**Title**: LDBFMINNM, LDBFMINNMA, LDBFMINNMAL, LDBFMINNML -- A64 | **Class**: `advsimd` | **XML ID**: `LDBFMINNM`

**Architecture**: `FEAT_LSFE` (ARMv9.6)

**Summary**: BFloat16 floating-point atomic minimum number in memory

**Description**:
This instruction atomically loads a 16-bit value from memory,
computes the BFloat16 minimum number with the value held in a register,
...
### Variant: `Floating-point (LDBFMINNM_16)` (No memory ordering)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `LDBFMINNM  <Hs>, <Ht>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------------|
| 00  111 1   00  A   R   1   Rs  0   111 00  Rn  Rt  |
```

#### Decode (A64.ldst.memop.LDBFMINNM_16)
...
#### Execute (A64.ldst.memop.LDBFMINNM_16)
...
bits(datasize) value;
bits(datasize) data;
constant AccessDescriptor accdesc = CreateAccDescFPAtomicOp(MemAtomicOp_BFMINNM, acquire,
                                                            release, tagchecked);
...
### Variant: `Floating-point (LDBFMINNMA_16)` (Acquire)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `LDBFMINNMA  <Hs>, <Ht>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Floating-point (LDBFMINNMAL_16)` (Acquire-release)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `LDBFMINNMAL  <Hs>, <Ht>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Floating-point (LDBFMINNML_16)` (Release)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `LDBFMINNML  <Hs>, <Ht>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
