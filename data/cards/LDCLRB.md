...
**Title**: LDCLRB, LDCLRAB, LDCLRALB, LDCLRLB -- A64 | **Class**: `general` | **XML ID**: `LDCLRB`

**Architecture**: `FEAT_LSE` (ARMv8.1)

**Summary**: Atomic bit clear on byte in memory
...
### Variant: `Integer (LDCLRB_32_memop)` (No memory ordering)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `LDCLRB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------------|
| 00  111 0   00  A   R   1   Rs  0   001 00  Rn  Rt  |
```

#### Decode (A64.ldst.memop.LDCLRB_32_memop)
...
#### Execute (A64.ldst.memop.LDCLRB_32_memop)
...
### Variant: `Integer (LDCLRAB_32_memop)` (Acquire)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `LDCLRAB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDCLRALB_32_memop)` (Acquire-release)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `LDCLRALB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDCLRLB_32_memop)` (Release)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `LDCLRLB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
