...
**Title**: LDEORH, LDEORAH, LDEORALH, LDEORLH -- A64 | **Class**: `general` | **XML ID**: `LDEORH`

**Architecture**: `FEAT_LSE` (ARMv8.1)

**Summary**: Atomic exclusive-OR on halfword in memory
...
### Variant: `Integer (LDEORH_32_memop)` (No memory ordering)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `LDEORH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------------|
| 01  111 0   00  A   R   1   Rs  0   010 00  Rn  Rt  |
```

#### Decode (A64.ldst.memop.LDEORH_32_memop)
...
#### Execute (A64.ldst.memop.LDEORH_32_memop)
...
### Variant: `Integer (LDEORAH_32_memop)` (Acquire)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `LDEORAH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDEORALH_32_memop)` (Acquire-release)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `LDEORALH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDEORLH_32_memop)` (Release)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `LDEORLH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
