...
**Title**: LDP -- A64 | **Class**: `general` | **XML ID**: `LDP_gen`
...
### Variant: `Post-index (LDP_32_ldstpair_post)` (32-bit)
- **Condition**: `opc == 00`
...
|-----------------------------|
| x0  101 0   001 1   imm7 Rt2 Rn  Rt  |
```

#### Decode (A64.ldst.ldstpair_post.LDP_32_ldstpair_post)
...
#### Postdecode (A64.ldst.ldstpair_post.LDP_32_ldstpair_post)
...
#### Execute (A64.ldst.ldstpair_post.LDP_32_ldstpair_post)
...
### Variant: `Post-index (LDP_64_ldstpair_post)` (64-bit)
- **Condition**: `opc == 10`
...
### Variant: `Pre-index (LDP_32_ldstpair_pre)` (32-bit)
- **Condition**: `opc == 00`
- **Assembly**: `LDP  <Wt1>, <Wt2>, [<Xn|SP>, #<imm>]!`
- **Fixed bits**: `opc`=`0`
- **Bit Pattern**: `???????????????????????????????0`
...
| 31  29  26 25  22 21  14   9   4  |
|-----------------------------|
| x0  101 0   011 1   imm7 Rt2 Rn  Rt  |
```

#### Decode (A64.ldst.ldstpair_pre.LDP_32_ldstpair_pre)
...
### Variant: `Pre-index (LDP_64_ldstpair_pre)` (64-bit)
- **Condition**: `opc == 10`
- **Assembly**: `LDP  <Xt1>, <Xt2>, [<Xn|SP>, #<imm>]!`
- **Fixed bits**: `opc`=`1`
- **Bit Pattern**: `???????????????????????????????1`
...
### Variant: `Signed offset (LDP_32_ldstpair_off)` (32-bit)
- **Condition**: `opc == 00`
- **Assembly**: `LDP  <Wt1>, <Wt2>, [<Xn|SP>{, #<imm>}]`
- **Fixed bits**: `opc`=`0`
- **Bit Pattern**: `???????????????????????????????0`
...
| 31  29  26 25  22 21  14   9   4  |
|-----------------------------|
| x0  101 0   010 1   imm7 Rt2 Rn  Rt  |
```

#### Decode (A64.ldst.ldstpair_off.LDP_32_ldstpair_off)
...
### Variant: `Signed offset (LDP_64_ldstpair_off)` (64-bit)
- **Condition**: `opc == 10`
- **Assembly**: `LDP  <Xt1>, <Xt2>, [<Xn|SP>{, #<imm>}]`
- **Fixed bits**: `opc`=`1`
- **Bit Pattern**: `???????????????????????????????1`
