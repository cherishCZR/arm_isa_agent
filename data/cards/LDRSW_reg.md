...
**Title**: LDRSW (register) -- A64 | **Class**: `general` | **XML ID**: `LDRSW_reg`

**Summary**: Load register signed word (register)
...
### Variant: `64-bit`
- **Assembly**: `LDRSW  <Xt>, [<Xn|SP>, (<Wm>|<Xm>){, <extend> {<amount>}}]`
**Encoding Diagram (32-bit)**:

```text
| 31  29  27 26 25 24 23  21 20  15  12 11   9   4  |
|--------------------------------------------|
| 10  11  1   0   0   0   10  1   Rm  option S   10  Rn  Rt  |
```

#### Decode (A64.ldst.ldst_regoff.LDRSW_64_ldst_regoff)

```
if option<1> == '0' then EndOfDecode(Decode_UNDEF);             // sub-word index
constant ExtendType extend_type = DecodeRegExtend(option);
constant integer shift = if S == '1' then 2 else 0;
```

#### Postdecode (A64.ldst.ldst_regoff.LDRSW_64_ldst_regoff)
...
#### Execute (A64.ldst.ldst_regoff.LDRSW_64_ldst_regoff)
