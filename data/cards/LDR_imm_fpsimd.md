...
**Title**: LDR (immediate, SIMD&FP) -- A64 | **Class**: `fpsimd` | **XML ID**: `LDR_imm_fpsimd`

**Architecture**: `FEAT_FP` (ARMv8.0)

**Summary**: Load SIMD&FP register (immediate offset)

**Description**:
This instruction loads an element from memory, and
writes the result as a scalar to the SIMD&FP register.
The address that is used for the load is calculated from a base register value,
a signed immediate offset, and an optional offset that is a multiple of the
element size.
...
### Variant: `Post-index (LDR_B_ldst_immpost)` (8-bit)
...
|--------------------------------|
| size 111 1   00  x1  0   imm9 01  Rn  Rt  |
```

#### Decode (A64.ldst.ldst_immpost.LDR_B_ldst_immpost)
...
#### Postdecode (A64.ldst.ldst_immpost.LDR_B_ldst_immpost)
...
#### Execute (A64.ldst.ldst_immpost.LDR_B_ldst_immpost)
...
### Variant: `Post-index (LDR_H_ldst_immpost)` (16-bit)
...
### Variant: `Post-index (LDR_S_ldst_immpost)` (32-bit)
...
### Variant: `Post-index (LDR_D_ldst_immpost)` (64-bit)
...
### Variant: `Post-index (LDR_Q_ldst_immpost)` (128-bit)
...
### Variant: `Pre-index (LDR_B_ldst_immpre)` (8-bit)
- **Condition**: `size == 00 && opc == 01`
- **Assembly**: `LDR  <Bt>, [<Xn|SP>, #<simm>]!`
- **Fixed bits**: `size`=`00`, `opc`=`0`
- **Bit Pattern**: `???????????????????????0??????00`
...
| 31  29  26 25  23  21 20  11   9   4  |
|--------------------------------|
| size 111 1   00  x1  0   imm9 11  Rn  Rt  |
```

#### Decode (A64.ldst.ldst_immpre.LDR_B_ldst_immpre)
...
### Variant: `Pre-index (LDR_H_ldst_immpre)` (16-bit)
- **Condition**: `size == 01 && opc == 01`
- **Assembly**: `LDR  <Ht>, [<Xn|SP>, #<simm>]!`
- **Fixed bits**: `size`=`01`, `opc`=`0`
- **Bit Pattern**: `???????????????????????0??????10`
...
### Variant: `Pre-index (LDR_S_ldst_immpre)` (32-bit)
- **Condition**: `size == 10 && opc == 01`
- **Assembly**: `LDR  <St>, [<Xn|SP>, #<simm>]!`
- **Fixed bits**: `size`=`10`, `opc`=`0`
- **Bit Pattern**: `???????????????????????0??????01`
...
### Variant: `Pre-index (LDR_D_ldst_immpre)` (64-bit)
- **Condition**: `size == 11 && opc == 01`
- **Assembly**: `LDR  <Dt>, [<Xn|SP>, #<simm>]!`
- **Fixed bits**: `size`=`11`, `opc`=`0`
- **Bit Pattern**: `???????????????????????0??????11`
...
### Variant: `Pre-index (LDR_Q_ldst_immpre)` (128-bit)
- **Condition**: `size == 00 && opc == 11`
- **Assembly**: `LDR  <Qt>, [<Xn|SP>, #<simm>]!`
- **Fixed bits**: `size`=`00`, `opc`=`1`
- **Bit Pattern**: `???????????????????????1??????00`
...
### Variant: `Unsigned offset (LDR_B_ldst_pos)` (8-bit)
- **Condition**: `size == 00 && opc == 01`
...
|--------------------------|
| size 111 1   01  x1  imm12 Rn  Rt  |
```

#### Decode (A64.ldst.ldst_pos.LDR_B_ldst_pos)
...
### Variant: `Unsigned offset (LDR_H_ldst_pos)` (16-bit)
- **Condition**: `size == 01 && opc == 01`
...
### Variant: `Unsigned offset (LDR_S_ldst_pos)` (32-bit)
- **Condition**: `size == 10 && opc == 01`
...
### Variant: `Unsigned offset (LDR_D_ldst_pos)` (64-bit)
- **Condition**: `size == 11 && opc == 01`
...
### Variant: `Unsigned offset (LDR_Q_ldst_pos)` (128-bit)
- **Condition**: `size == 00 && opc == 11`
