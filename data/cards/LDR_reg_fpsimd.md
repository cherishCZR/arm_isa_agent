...
**Title**: LDR (register, SIMD&FP) -- A64 | **Class**: `fpsimd` | **XML ID**: `LDR_reg_fpsimd`

**Architecture**: `FEAT_FP` (ARMv8.0)

**Summary**: Load SIMD&FP register (register offset)
...
### Variant: `SIMD&FP registers (LDR_B_ldst_regoff)` (8-bit)
- **Condition**: `size == 00 && opc == 01 && option != 011`
- **Assembly**: `LDR  <Bt>, [<Xn|SP>, (<Wm>|<Xm>), <extend> {<amount>}]`
...
|--------------------------------------|
| size 111 1   00  x1  1   Rm  option S   10  Rn  Rt  |
```

#### Decode (A64.ldst.ldst_regoff.LDR_B_ldst_regoff)
...
#### Postdecode (A64.ldst.ldst_regoff.LDR_B_ldst_regoff)
...
#### Execute (A64.ldst.ldst_regoff.LDR_B_ldst_regoff)
...
### Variant: `SIMD&FP registers (LDR_BL_ldst_regoff)` (8-bit)
- **Condition**: `size == 00 && opc == 01 && option == 011`
- **Assembly**: `LDR  <Bt>, [<Xn|SP>, <Xm>{, LSL <amount>}]`
...
### Variant: `SIMD&FP registers (LDR_H_ldst_regoff)` (16-bit)
- **Condition**: `size == 01 && opc == 01`
- **Assembly**: `LDR  <Ht>, [<Xn|SP>, (<Wm>|<Xm>){, <extend> {<amount>}}]`
...
### Variant: `SIMD&FP registers (LDR_S_ldst_regoff)` (32-bit)
- **Condition**: `size == 10 && opc == 01`
- **Assembly**: `LDR  <St>, [<Xn|SP>, (<Wm>|<Xm>){, <extend> {<amount>}}]`
...
### Variant: `SIMD&FP registers (LDR_D_ldst_regoff)` (64-bit)
- **Condition**: `size == 11 && opc == 01`
- **Assembly**: `LDR  <Dt>, [<Xn|SP>, (<Wm>|<Xm>){, <extend> {<amount>}}]`
...
### Variant: `SIMD&FP registers (LDR_Q_ldst_regoff)` (128-bit)
- **Condition**: `size == 00 && opc == 11`
- **Assembly**: `LDR  <Qt>, [<Xn|SP>, (<Wm>|<Xm>){, <extend> {<amount>}}]`
