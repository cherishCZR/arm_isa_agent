...
**Title**: LDSMAXH, LDSMAXAH, LDSMAXALH, LDSMAXLH -- A64 | **Class**: `general` | **XML ID**: `LDSMAXH`

**Architecture**: `FEAT_LSE` (ARMv8.1)

**Summary**: Atomic signed maximum on halfword in memory
...
### Variant: `Integer (LDSMAXH_32_memop)` (No memory ordering)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `LDSMAXH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------------|
| 01  111 0   00  A   R   1   Rs  0   100 00  Rn  Rt  |
```

#### Decode (A64.ldst.memop.LDSMAXH_32_memop)
...
#### Execute (A64.ldst.memop.LDSMAXH_32_memop)
...
### Variant: `Integer (LDSMAXAH_32_memop)` (Acquire)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `LDSMAXAH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDSMAXALH_32_memop)` (Acquire-release)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `LDSMAXALH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDSMAXLH_32_memop)` (Release)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `LDSMAXLH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
