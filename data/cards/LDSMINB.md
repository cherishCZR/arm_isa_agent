...
**Title**: LDSMINB, LDSMINAB, LDSMINALB, LDSMINLB -- A64 | **Class**: `general` | **XML ID**: `LDSMINB`

**Architecture**: `FEAT_LSE` (ARMv8.1)

**Summary**: Atomic signed minimum on byte in memory
...
### Variant: `Integer (LDSMINB_32_memop)` (No memory ordering)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `LDSMINB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------------|
| 00  111 0   00  A   R   1   Rs  0   101 00  Rn  Rt  |
```

#### Decode (A64.ldst.memop.LDSMINB_32_memop)
...
#### Execute (A64.ldst.memop.LDSMINB_32_memop)
...
### Variant: `Integer (LDSMINAB_32_memop)` (Acquire)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `LDSMINAB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDSMINALB_32_memop)` (Acquire-release)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `LDSMINALB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDSMINLB_32_memop)` (Release)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `LDSMINLB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
