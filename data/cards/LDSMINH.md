...
**Title**: LDSMINH, LDSMINAH, LDSMINALH, LDSMINLH -- A64 | **Class**: `general` | **XML ID**: `LDSMINH`

**Architecture**: `FEAT_LSE` (ARMv8.1)

**Summary**: Atomic signed minimum on halfword in memory
...
### Variant: `Integer (LDSMINH_32_memop)` (No memory ordering)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `LDSMINH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------------|
| 01  111 0   00  A   R   1   Rs  0   101 00  Rn  Rt  |
```

#### Decode (A64.ldst.memop.LDSMINH_32_memop)
...
#### Execute (A64.ldst.memop.LDSMINH_32_memop)
...
### Variant: `Integer (LDSMINAH_32_memop)` (Acquire)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `LDSMINAH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDSMINALH_32_memop)` (Acquire-release)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `LDSMINALH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDSMINLH_32_memop)` (Release)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `LDSMINLH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
