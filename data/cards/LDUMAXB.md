...
**Title**: LDUMAXB, LDUMAXAB, LDUMAXALB, LDUMAXLB -- A64 | **Class**: `general` | **XML ID**: `LDUMAXB`

**Architecture**: `FEAT_LSE` (ARMv8.1)

**Summary**: Atomic unsigned maximum on byte in memory
...
### Variant: `Integer (LDUMAXB_32_memop)` (No memory ordering)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `LDUMAXB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------------|
| 00  111 0   00  A   R   1   Rs  0   110 00  Rn  Rt  |
```

#### Decode (A64.ldst.memop.LDUMAXB_32_memop)
...
#### Execute (A64.ldst.memop.LDUMAXB_32_memop)
...
### Variant: `Integer (LDUMAXAB_32_memop)` (Acquire)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `LDUMAXAB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDUMAXALB_32_memop)` (Acquire-release)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `LDUMAXALB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDUMAXLB_32_memop)` (Release)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `LDUMAXLB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
