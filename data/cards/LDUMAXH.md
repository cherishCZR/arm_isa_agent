...
**Title**: LDUMAXH, LDUMAXAH, LDUMAXALH, LDUMAXLH -- A64 | **Class**: `general` | **XML ID**: `LDUMAXH`

**Architecture**: `FEAT_LSE` (ARMv8.1)

**Summary**: Atomic unsigned maximum on halfword in memory
...
### Variant: `Integer (LDUMAXH_32_memop)` (No memory ordering)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `LDUMAXH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------------|
| 01  111 0   00  A   R   1   Rs  0   110 00  Rn  Rt  |
```

#### Decode (A64.ldst.memop.LDUMAXH_32_memop)
...
#### Execute (A64.ldst.memop.LDUMAXH_32_memop)
...
### Variant: `Integer (LDUMAXAH_32_memop)` (Acquire)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `LDUMAXAH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDUMAXALH_32_memop)` (Acquire-release)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `LDUMAXALH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDUMAXLH_32_memop)` (Release)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `LDUMAXLH  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
