...
**Title**: LDUMINB, LDUMINAB, LDUMINALB, LDUMINLB -- A64 | **Class**: `general` | **XML ID**: `LDUMINB`

**Architecture**: `FEAT_LSE` (ARMv8.1)

**Summary**: Atomic unsigned minimum on byte in memory
...
### Variant: `Integer (LDUMINB_32_memop)` (No memory ordering)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `LDUMINB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|-----------------------------------------|
| 00  111 0   00  A   R   1   Rs  0   111 00  Rn  Rt  |
```

#### Decode (A64.ldst.memop.LDUMINB_32_memop)
...
#### Execute (A64.ldst.memop.LDUMINB_32_memop)
...
### Variant: `Integer (LDUMINAB_32_memop)` (Acquire)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `LDUMINAB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDUMINALB_32_memop)` (Acquire-release)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `LDUMINALB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (LDUMINLB_32_memop)` (Release)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `LDUMINLB  <Ws>, <Wt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
