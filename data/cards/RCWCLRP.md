...
**Title**: RCWCLRP, RCWCLRPA, RCWCLRPAL, RCWCLRPL -- A64 | **Class**: `general` | **XML ID**: `RCWCLRP`

**Architecture**: `FEAT_D128 && FEAT_THE` (FEAT_D128 && FEAT_THE)

**Summary**: Read check write atomic bit clear on quadword in memory
...
### Variant: `Integer (RCWCLRP_128_memop_128)` (RCWCLRP)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `RCWCLRP  <Xt1>, <Xt2>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|--------------------------------------|
| 0   0   011001 A   R   1   Rt2 1   001 00  Rn  Rt  |
```

#### Decode (A64.ldst.memop_128.RCWCLRP_128_memop_128)
...
#### Execute (A64.ldst.memop_128.RCWCLRP_128_memop_128)
...
### Variant: `Integer (RCWCLRPA_128_memop_128)` (RCWCLRPA)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `RCWCLRPA  <Xt1>, <Xt2>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (RCWCLRPAL_128_memop_128)` (RCWCLRPAL)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `RCWCLRPAL  <Xt1>, <Xt2>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (RCWCLRPL_128_memop_128)` (RCWCLRPL)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `RCWCLRPL  <Xt1>, <Xt2>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
