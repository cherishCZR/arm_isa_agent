...
**Title**: RCWSCASP, RCWSCASPA, RCWSCASPAL, RCWSCASPL -- A64 | **Class**: `general` | **XML ID**: `RCWSCASP`

**Architecture**: `FEAT_D128 && FEAT_THE` (FEAT_D128 && FEAT_THE)

**Summary**: Read check write software compare and swap quadword in memory
...
### Variant: `Integer (RCWSCASP_C64_rcwcomswappr)` (RCWSCASP)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `RCWSCASP  <Xs>, <X(s+1)>, <Xt>, <X(t+1)>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|--------------------------------|
| 0   1   011001 A   R   1   Rs  000011 Rn  Rt  |
```

#### Decode (A64.ldst.rcwcomswappr.RCWSCASP_C64_rcwcomswappr)
...
#### Execute (A64.ldst.rcwcomswappr.RCWSCASP_C64_rcwcomswappr)
...
### Variant: `Integer (RCWSCASPA_C64_rcwcomswappr)` (RCWSCASPA)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `RCWSCASPA  <Xs>, <X(s+1)>, <Xt>, <X(t+1)>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (RCWSCASPAL_C64_rcwcomswappr)` (RCWSCASPAL)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `RCWSCASPAL  <Xs>, <X(s+1)>, <Xt>, <X(t+1)>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (RCWSCASPL_C64_rcwcomswappr)` (RCWSCASPL)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `RCWSCASPL  <Xs>, <X(s+1)>, <Xt>, <X(t+1)>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
