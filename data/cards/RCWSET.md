...
**Title**: RCWSET, RCWSETA, RCWSETAL, RCWSETL -- A64 | **Class**: `general` | **XML ID**: `RCWSET`

**Architecture**: `FEAT_THE` (ARMv8.9)

**Summary**: Read check write atomic bit set on doubleword in memory
...
### Variant: `Integer (RCWSET_64_memop)` (RCWSET)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `RCWSET  <Xs>, <Xt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|--------------------------------------------|
| 0   0   111 0   00  A   R   1   Rs  1   011 00  Rn  Rt  |
```

#### Decode (A64.ldst.memop.RCWSET_64_memop)
...
#### Execute (A64.ldst.memop.RCWSET_64_memop)
...
### Variant: `Integer (RCWSETA_64_memop)` (RCWSETA)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `RCWSETA  <Xs>, <Xt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (RCWSETAL_64_memop)` (RCWSETAL)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `RCWSETAL  <Xs>, <Xt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (RCWSETL_64_memop)` (RCWSETL)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `RCWSETL  <Xs>, <Xt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
