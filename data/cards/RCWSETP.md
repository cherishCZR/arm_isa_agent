...
**Title**: RCWSETP, RCWSETPA, RCWSETPAL, RCWSETPL -- A64 | **Class**: `general` | **XML ID**: `RCWSETP`

**Architecture**: `FEAT_D128 && FEAT_THE` (FEAT_D128 && FEAT_THE)

**Summary**: Read check write atomic bit set on quadword in memory
...
### Variant: `Integer (RCWSETP_128_memop_128)` (RCWSETP)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `RCWSETP  <Xt1>, <Xt2>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|--------------------------------------|
| 0   0   011001 A   R   1   Rt2 1   011 00  Rn  Rt  |
```

#### Decode (A64.ldst.memop_128.RCWSETP_128_memop_128)
...
#### Execute (A64.ldst.memop_128.RCWSETP_128_memop_128)
...
### Variant: `Integer (RCWSETPA_128_memop_128)` (RCWSETPA)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `RCWSETPA  <Xt1>, <Xt2>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (RCWSETPAL_128_memop_128)` (RCWSETPAL)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `RCWSETPAL  <Xt1>, <Xt2>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (RCWSETPL_128_memop_128)` (RCWSETPL)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `RCWSETPL  <Xt1>, <Xt2>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
