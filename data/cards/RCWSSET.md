...
**Title**: RCWSSET, RCWSSETA, RCWSSETAL, RCWSSETL -- A64 | **Class**: `general` | **XML ID**: `RCWSSET`

**Architecture**: `FEAT_THE` (ARMv8.9)

**Summary**: Read check write software atomic bit set on doubleword in memory

**Description**:
This instruction atomically loads
a 64-bit doubleword from memory, performs a bitwise OR with the complement of the value
...
### Variant: `Integer (RCWSSET_64_memop)` (RCWSSET)
- **Condition**: `A == 0 && R == 0`
- **Assembly**: `RCWSSET  <Xs>, <Xt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????00????????`
...
|--------------------------------------------|
| 0   1   111 0   00  A   R   1   Rs  1   011 00  Rn  Rt  |
```

#### Decode (A64.ldst.memop.RCWSSET_64_memop)
...
#### Execute (A64.ldst.memop.RCWSSET_64_memop)
...
### Variant: `Integer (RCWSSETA_64_memop)` (RCWSSETA)
- **Condition**: `A == 1 && R == 0`
- **Assembly**: `RCWSSETA  <Xs>, <Xt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????01????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (RCWSSETAL_64_memop)` (RCWSSETAL)
- **Condition**: `A == 1 && R == 1`
- **Assembly**: `RCWSSETAL  <Xs>, <Xt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????11????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (RCWSSETL_64_memop)` (RCWSSETL)
- **Condition**: `A == 0 && R == 1`
- **Assembly**: `RCWSSETL  <Xs>, <Xt>, [<Xn|SP>]`
- **Fixed bits**: `A`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????10????????`
**Encoding Diagram (32-bit)**:
