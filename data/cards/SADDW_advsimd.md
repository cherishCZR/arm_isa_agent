...
**Title**: SADDW, SADDW2 -- A64 | **Class**: `advsimd` | **XML ID**: `SADDW_advsimd`

**Architecture**: `FEAT_AdvSIMD` (ARMv8.0)

**Summary**: Signed add wide

**Description**:
This instruction adds vector elements of the
first source SIMD&FP register to the corresponding
vector elements in the lower or upper half of the
second source SIMD&FP register,
places the results in a vector,
and writes the vector to the SIMD&FP destination register.

The SADDW instruction extracts
the second source vector from the lower half
of the second source register. The SADDW2 instruction extracts
...
- **Assembly**: `SADDW{2}  <Vd>.<Ta>, <Vn>.<Ta>, <Vm>.<Tb>`
**Encoding Diagram (32-bit)**:

```text
| 31 30 29 28 27  24 23  21 20  15  13 12 11   9   4  |
|-----------------------------------------------|
| 0   Q   0   0   111 0   size 1   Rm  00  0   1   00  Rn  Rd  |
```

#### Decode (A64.simd_dp.asimddiff.SADDW_asimddiff_W)
...
#### Execute (A64.simd_dp.asimddiff.SADDW_asimddiff_W)
