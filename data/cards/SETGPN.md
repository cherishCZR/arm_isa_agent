...
**Title**: SETGPN, SETGMN, SETGEN -- A64 | **Class**: `general` | **XML ID**: `SETGPN`

**Architecture**: `FEAT_MOPS && FEAT_MTE` (FEAT_MOPS && FEAT_MTE)

**Summary**: Memory set with tag setting, non-temporal
...
SETGMN instruction, and sets an IMPLEMENTATION DEFINED portion of the requested number
of bytes. SETGMN sets a further IMPLEMENTATION DEFINED portion of the remaining bytes.
SETGEN sets any final remaining bytes.
...
### Variant: `Integer (SETGPN_SET_memcms)` (Prologue)
- **Condition**: `op2 == 0010`
- **Assembly**: `SETGPN  [<Xd>]!, <Xn>!, <Xs>`
- **Fixed bits**: `op2`=`00`
- **Bit Pattern**: `??????????????00????????????????`
...
|-----------------------------------|
| sz  011 1   01  11  0   Rs  xx10 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.SETGPN_SET_memcms)
...
#### Execute (A64.ldst.memcms.SETGPN_SET_memcms)
...
### Variant: `Integer (SETGMN_SET_memcms)` (Main)
- **Condition**: `op2 == 0110`
- **Assembly**: `SETGMN  [<Xd>]!, <Xn>!, <Xs>`
- **Fixed bits**: `op2`=`01`
- **Bit Pattern**: `??????????????10????????????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (SETGEN_SET_memcms)` (Epilogue)
- **Condition**: `op2 == 1010`
- **Assembly**: `SETGEN  [<Xd>]!, <Xn>!, <Xs>`
- **Fixed bits**: `op2`=`10`
- **Bit Pattern**: `??????????????01????????????????`
**Encoding Diagram (32-bit)**:
