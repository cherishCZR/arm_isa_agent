...
**Title**: SETGPT, SETGMT, SETGET -- A64 | **Class**: `general` | **XML ID**: `SETGPT`

**Architecture**: `FEAT_MOPS && FEAT_MTE` (FEAT_MOPS && FEAT_MTE)

**Summary**: Memory set with tag setting, unprivileged
...
SETGMT instruction, and sets an IMPLEMENTATION DEFINED portion of the requested number
of bytes. SETGMT sets a further IMPLEMENTATION DEFINED portion of the remaining bytes.
SETGET sets any final remaining bytes.
...
### Variant: `Integer (SETGPT_SET_memcms)` (Prologue)
- **Condition**: `op2 == 0001`
- **Assembly**: `SETGPT  [<Xd>]!, <Xn>!, <Xs>`
- **Fixed bits**: `op2`=`00`
- **Bit Pattern**: `??????????????00????????????????`
...
|-----------------------------------|
| sz  011 1   01  11  0   Rs  xx01 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.SETGPT_SET_memcms)
...
#### Execute (A64.ldst.memcms.SETGPT_SET_memcms)
...
### Variant: `Integer (SETGMT_SET_memcms)` (Main)
- **Condition**: `op2 == 0101`
- **Assembly**: `SETGMT  [<Xd>]!, <Xn>!, <Xs>`
- **Fixed bits**: `op2`=`01`
- **Bit Pattern**: `??????????????10????????????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (SETGET_SET_memcms)` (Epilogue)
- **Condition**: `op2 == 1001`
- **Assembly**: `SETGET  [<Xd>]!, <Xn>!, <Xs>`
- **Fixed bits**: `op2`=`10`
- **Bit Pattern**: `??????????????01????????????????`
**Encoding Diagram (32-bit)**:
