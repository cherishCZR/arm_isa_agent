...
**Title**: SETP, SETM, SETE -- A64 | **Class**: `general` | **XML ID**: `SETP`
...
SETM instruction, and sets an IMPLEMENTATION DEFINED portion of the requested number
of bytes. SETM sets a further IMPLEMENTATION DEFINED portion of the remaining bytes.
SETE sets any final remaining bytes.
...
### Variant: `Integer (SETP_SET_memcms)` (Prologue)
- **Condition**: `op2 == 0000`
- **Assembly**: `SETP  [<Xd>]!, <Xn>!, <Xs>`
- **Fixed bits**: `op2`=`00`
- **Bit Pattern**: `??????????????00????????????????`
...
|-----------------------------------|
| sz  011 0   01  11  0   Rs  xx00 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.SETP_SET_memcms)
...
#### Execute (A64.ldst.memcms.SETP_SET_memcms)
...
### Variant: `Integer (SETM_SET_memcms)` (Main)
- **Condition**: `op2 == 0100`
- **Assembly**: `SETM  [<Xd>]!, <Xn>!, <Xs>`
- **Fixed bits**: `op2`=`01`
- **Bit Pattern**: `??????????????10????????????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (SETE_SET_memcms)` (Epilogue)
- **Condition**: `op2 == 1000`
- **Assembly**: `SETE  [<Xd>]!, <Xn>!, <Xs>`
- **Fixed bits**: `op2`=`10`
- **Bit Pattern**: `??????????????01????????????????`
**Encoding Diagram (32-bit)**:
