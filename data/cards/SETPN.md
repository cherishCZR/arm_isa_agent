...
**Title**: SETPN, SETMN, SETEN -- A64 | **Class**: `general` | **XML ID**: `SETPN`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory set, non-temporal
...
SETMN instruction, and sets an IMPLEMENTATION DEFINED portion of the requested number
of bytes. SETMN sets a further IMPLEMENTATION DEFINED portion of the remaining bytes.
SETEN sets any final remaining bytes.
...
### Variant: `Integer (SETPN_SET_memcms)` (Prologue)
- **Condition**: `op2 == 0010`
- **Assembly**: `SETPN  [<Xd>]!, <Xn>!, <Xs>`
- **Fixed bits**: `op2`=`00`
- **Bit Pattern**: `??????????????00????????????????`
...
|-----------------------------------|
| sz  011 0   01  11  0   Rs  xx10 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.SETPN_SET_memcms)
...
#### Execute (A64.ldst.memcms.SETPN_SET_memcms)
...
### Variant: `Integer (SETMN_SET_memcms)` (Main)
- **Condition**: `op2 == 0110`
- **Assembly**: `SETMN  [<Xd>]!, <Xn>!, <Xs>`
- **Fixed bits**: `op2`=`01`
- **Bit Pattern**: `??????????????10????????????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (SETEN_SET_memcms)` (Epilogue)
- **Condition**: `op2 == 1010`
- **Assembly**: `SETEN  [<Xd>]!, <Xn>!, <Xs>`
- **Fixed bits**: `op2`=`10`
- **Bit Pattern**: `??????????????01????????????????`
**Encoding Diagram (32-bit)**:
