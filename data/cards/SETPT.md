...
**Title**: SETPT, SETMT, SETET -- A64 | **Class**: `general` | **XML ID**: `SETPT`

**Architecture**: `FEAT_MOPS` (ARMv8.8)

**Summary**: Memory set, unprivileged
...
SETMT instruction, and sets an IMPLEMENTATION DEFINED portion of the requested number
of bytes. SETMT sets a further IMPLEMENTATION DEFINED portion of the remaining bytes.
SETET sets any final remaining bytes.
...
### Variant: `Integer (SETPT_SET_memcms)` (Prologue)
- **Condition**: `op2 == 0001`
- **Assembly**: `SETPT  [<Xd>]!, <Xn>!, <Xs>`
- **Fixed bits**: `op2`=`00`
- **Bit Pattern**: `??????????????00????????????????`
...
|-----------------------------------|
| sz  011 0   01  11  0   Rs  xx01 01  Rn  Rd  |
```

#### Decode (A64.ldst.memcms.SETPT_SET_memcms)
...
#### Execute (A64.ldst.memcms.SETPT_SET_memcms)
...
### Variant: `Integer (SETMT_SET_memcms)` (Main)
- **Condition**: `op2 == 0101`
- **Assembly**: `SETMT  [<Xd>]!, <Xn>!, <Xs>`
- **Fixed bits**: `op2`=`01`
- **Bit Pattern**: `??????????????10????????????????`
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (SETET_SET_memcms)` (Epilogue)
- **Condition**: `op2 == 1001`
- **Assembly**: `SETET  [<Xd>]!, <Xn>!, <Xs>`
- **Fixed bits**: `op2`=`10`
- **Bit Pattern**: `??????????????01????????????????`
**Encoding Diagram (32-bit)**:
