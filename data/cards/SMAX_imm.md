...
**Title**: SMAX (immediate) -- A64 | **Class**: `general` | **XML ID**: `SMAX_imm`

**Architecture**: `FEAT_CSSC` (ARMv8.9)

**Summary**: Signed maximum (immediate)

**Description**:
This instruction determines the signed maximum of the
source register value and immediate, and writes the result to the
destination register.

### Variant: `Integer (SMAX_32_minmax_imm)` (32-bit)
- **Condition**: `sf == 0`
- **Assembly**: `SMAX  <Wd>, <Wn>, #<simm>`
- **Fixed bits**: `sf`=`0`
- **Bit Pattern**: `???????????????????????????????0`
...
|--------------------------|
| sf  0   0   1000111 0000 imm8 Rn  Rd  |
```

#### Decode (A64.dpimm.minmax_imm.SMAX_32_minmax_imm)
...
#### Execute (A64.dpimm.minmax_imm.SMAX_32_minmax_imm)

```
constant bits(datasize) operand1 = X[n, datasize];
constant integer result = Max(SInt(operand1), imm);
...
### Variant: `Integer (SMAX_64_minmax_imm)` (64-bit)
- **Condition**: `sf == 1`
- **Assembly**: `SMAX  <Xd>, <Xn>, #<simm>`
- **Fixed bits**: `sf`=`1`
- **Bit Pattern**: `???????????????????????????????1`
**Encoding Diagram (32-bit)**:
