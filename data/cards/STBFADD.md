...
**Title**: STBFADD, STBFADDL -- A64 | **Class**: `advsimd` | **XML ID**: `STBFADD`

**Architecture**: `FEAT_LSFE` (ARMv9.6)

**Summary**: BFloat16 floating-point atomic add in memory, without return
...
### Variant: `Floating-point (STBFADD_16)` (No memory ordering)
- **Condition**: `R == 0`
- **Assembly**: `STBFADD  <Hs>, [<Xn|SP>]`
- **Fixed bits**: `R`=`0`
- **Bit Pattern**: `??????????????????????0?????????`
...
|-----------------------------------------|
| 00  111 1   00  0   R   1   Rs  1   000 00  Rn  11111 |
```

#### Decode (A64.ldst.memop.STBFADD_16)
...
#### Execute (A64.ldst.memop.STBFADD_16)
...
### Variant: `Floating-point (STBFADDL_16)` (Release)
- **Condition**: `R == 1`
- **Assembly**: `STBFADDL  <Hs>, [<Xn|SP>]`
- **Fixed bits**: `R`=`1`
- **Bit Pattern**: `??????????????????????1?????????`
**Encoding Diagram (32-bit)**:
