...
**Title**: STBFMAXNM, STBFMAXNML -- A64 | **Class**: `advsimd` | **XML ID**: `STBFMAXNM`

**Architecture**: `FEAT_LSFE` (ARMv9.6)

**Summary**: BFloat16 floating-point atomic maximum number in memory, without return
...
### Variant: `Floating-point (STBFMAXNM_16)` (No memory ordering)
- **Condition**: `R == 0`
- **Assembly**: `STBFMAXNM  <Hs>, [<Xn|SP>]`
- **Fixed bits**: `R`=`0`
- **Bit Pattern**: `??????????????????????0?????????`
...
|-----------------------------------------|
| 00  111 1   00  0   R   1   Rs  1   110 00  Rn  11111 |
```

#### Decode (A64.ldst.memop.STBFMAXNM_16)
...
#### Execute (A64.ldst.memop.STBFMAXNM_16)
...
### Variant: `Floating-point (STBFMAXNML_16)` (Release)
- **Condition**: `R == 1`
- **Assembly**: `STBFMAXNML  <Hs>, [<Xn|SP>]`
- **Fixed bits**: `R`=`1`
- **Bit Pattern**: `??????????????????????1?????????`
**Encoding Diagram (32-bit)**:
