...
**Title**: STBFMIN, STBFMINL -- A64 | **Class**: `advsimd` | **XML ID**: `STBFMIN`

**Architecture**: `FEAT_LSFE` (ARMv9.6)

**Summary**: BFloat16 floating-point atomic minimum in memory, without return
...
### Variant: `Floating-point (STBFMIN_16)` (No memory ordering)
- **Condition**: `R == 0`
- **Assembly**: `STBFMIN  <Hs>, [<Xn|SP>]`
- **Fixed bits**: `R`=`0`
- **Bit Pattern**: `??????????????????????0?????????`
...
|-----------------------------------------|
| 00  111 1   00  0   R   1   Rs  1   101 00  Rn  11111 |
```

#### Decode (A64.ldst.memop.STBFMIN_16)
...
#### Execute (A64.ldst.memop.STBFMIN_16)
...
### Variant: `Floating-point (STBFMINL_16)` (Release)
- **Condition**: `R == 1`
- **Assembly**: `STBFMINL  <Hs>, [<Xn|SP>]`
- **Fixed bits**: `R`=`1`
- **Bit Pattern**: `??????????????????????1?????????`
**Encoding Diagram (32-bit)**:
