...
**Title**: STBFMINNM, STBFMINNML -- A64 | **Class**: `advsimd` | **XML ID**: `STBFMINNM`

**Architecture**: `FEAT_LSFE` (ARMv9.6)

**Summary**: BFloat16 floating-point atomic minimum number in memory, without return
...
### Variant: `Floating-point (STBFMINNM_16)` (No memory ordering)
- **Condition**: `R == 0`
- **Assembly**: `STBFMINNM  <Hs>, [<Xn|SP>]`
- **Fixed bits**: `R`=`0`
- **Bit Pattern**: `??????????????????????0?????????`
...
|-----------------------------------------|
| 00  111 1   00  0   R   1   Rs  1   111 00  Rn  11111 |
```

#### Decode (A64.ldst.memop.STBFMINNM_16)
...
#### Execute (A64.ldst.memop.STBFMINNM_16)
...
### Variant: `Floating-point (STBFMINNML_16)` (Release)
- **Condition**: `R == 1`
- **Assembly**: `STBFMINNML  <Hs>, [<Xn|SP>]`
- **Fixed bits**: `R`=`1`
- **Bit Pattern**: `??????????????????????1?????????`
**Encoding Diagram (32-bit)**:
