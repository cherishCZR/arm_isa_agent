## STEOR_LDEOR `[ALIAS]`
_ARM A64 Instruction_ (Alias of ldeor.xml)

**Title**: STEOR, STEORL -- A64 | **Class**: `general` | **XML ID**: `STEOR_LDEOR`

**Architecture**: `FEAT_LSE` (ARMv8.1)

**Summary**: Atomic exclusive-OR on word or doubleword in memory, without return
...
### Variant: `Integer (STEOR_LDEOR_32_memop)` (32-bit no memory ordering)
- **Condition**: `size == 10 && R == 0`
- **Assembly**: `STEOR  <Ws>, [<Xn|SP>]`
- **Fixed bits**: `size`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????0???????0?`
- **Alias of**: `LDEOR  <Ws>, WZR, [<Xn|SP>]`
...
|-----------------------------------------|
| 1x  111 0   00  0   R   1   Rs  0   010 00  Rn  11111 |
```

### Variant: `Integer (STEORL_LDEORL_32_memop)` (32-bit release)
- **Condition**: `size == 10 && R == 1`
- **Assembly**: `STEORL  <Ws>, [<Xn|SP>]`
- **Fixed bits**: `size`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????1???????0?`
- **Alias of**: `LDEORL  <Ws>, WZR, [<Xn|SP>]`
  Condition: Unconditionally
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (STEOR_LDEOR_64_memop)` (64-bit no memory ordering)
- **Condition**: `size == 11 && R == 0`
- **Assembly**: `STEOR  <Xs>, [<Xn|SP>]`
- **Fixed bits**: `size`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????0???????1?`
- **Alias of**: `LDEOR  <Xs>, XZR, [<Xn|SP>]`
  Condition: Unconditionally
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (STEORL_LDEORL_64_memop)` (64-bit release)
- **Condition**: `size == 11 && R == 1`
- **Assembly**: `STEORL  <Xs>, [<Xn|SP>]`
- **Fixed bits**: `size`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????1???????1?`
- **Alias of**: `LDEORL  <Xs>, XZR, [<Xn|SP>]`
  Condition: Unconditionally
**Encoding Diagram (32-bit)**:
