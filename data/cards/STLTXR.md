...
**Title**: STLTXR -- A64 | **Class**: `general` | **XML ID**: `STLTXR`

**Architecture**: `FEAT_LSUI` (ARMv9.6)

**Summary**: Store-release unprivileged exclusive register
...
### Variant: `No offset (STLTXR_SR32_ldstexclr_unpriv)` (32-bit)
- **Condition**: `sz == 0`
- **Assembly**: `STLTXR  <Ws>, <Wt>, [<Xn|SP>{, #0}]`
- **Fixed bits**: `sz`=`0`
- **Bit Pattern**: `??????????????????????????????0?`
...
|--------------------------------|
| 1   sz  0010010 0   0   Rs  1   (1)(1)(1)(1)(1) Rn  Rt  |
```

#### Decode (A64.ldst.ldstexclr_unpriv.STLTXR_SR32_ldstexclr_unpriv)
...
#### Execute (A64.ldst.ldstexclr_unpriv.STLTXR_SR32_ldstexclr_unpriv)
...
### Variant: `No offset (STLTXR_SR64_ldstexclr_unpriv)` (64-bit)
- **Condition**: `sz == 1`
- **Assembly**: `STLTXR  <Ws>, <Xt>, [<Xn|SP>{, #0}]`
- **Fixed bits**: `sz`=`1`
- **Bit Pattern**: `??????????????????????????????1?`
**Encoding Diagram (32-bit)**:
