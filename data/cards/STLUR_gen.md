...
**Title**: STLUR -- A64 | **Class**: `general` | **XML ID**: `STLUR_gen`

**Architecture**: `FEAT_LRCPC2` (ARMv8.4)

**Summary**: Store-release register (unscaled)
...
### Variant: `Unscaled offset (STLUR_32_ldapstl_unscaled)` (32-bit)
- **Condition**: `size == 10`
- **Assembly**: `STLUR  <Wt>, [<Xn|SP>{, #<simm>}]`
- **Fixed bits**: `size`=`0`
- **Bit Pattern**: `??????????????????????????????0?`
...
|--------------------------|
| 1x  011001 00  0   imm9 00  Rn  Rt  |
```

#### Decode (A64.ldst.ldapstl_unscaled.STLUR_32_ldapstl_unscaled)
...
#### Execute (A64.ldst.ldapstl_unscaled.STLUR_32_ldapstl_unscaled)
...
### Variant: `Unscaled offset (STLUR_64_ldapstl_unscaled)` (64-bit)
- **Condition**: `size == 11`
- **Assembly**: `STLUR  <Xt>, [<Xn|SP>{, #<simm>}]`
- **Fixed bits**: `size`=`1`
- **Bit Pattern**: `??????????????????????????????1?`
**Encoding Diagram (32-bit)**:
