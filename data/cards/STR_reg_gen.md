...
**Title**: STR (register) -- A64 | **Class**: `general` | **XML ID**: `STR_reg_gen`

**Summary**: Store register (register)
...
### Variant: `Integer (STR_32_ldst_regoff)` (32-bit)
- **Condition**: `size == 10`
- **Assembly**: `STR  <Wt>, [<Xn|SP>, (<Wm>|<Xm>){, <extend> {<amount>}}]`
- **Fixed bits**: `size`=`0`
- **Bit Pattern**: `??????????????????????????????0?`
...
|--------------------------------------|
| 1x  111 0   00  00  1   Rm  option S   10  Rn  Rt  |
```

#### Decode (A64.ldst.ldst_regoff.STR_32_ldst_regoff)
...
#### Postdecode (A64.ldst.ldst_regoff.STR_32_ldst_regoff)
...
#### Execute (A64.ldst.ldst_regoff.STR_32_ldst_regoff)
...
### Variant: `Integer (STR_64_ldst_regoff)` (64-bit)
- **Condition**: `size == 11`
- **Assembly**: `STR  <Xt>, [<Xn|SP>, (<Wm>|<Xm>){, <extend> {<amount>}}]`
- **Fixed bits**: `size`=`1`
- **Bit Pattern**: `??????????????????????????????1?`
**Encoding Diagram (32-bit)**:
