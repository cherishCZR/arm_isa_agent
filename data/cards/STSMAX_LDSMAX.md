## STSMAX_LDSMAX `[ALIAS]`
_ARM A64 Instruction_ (Alias of ldsmax.xml)

**Title**: STSMAX, STSMAXL -- A64 | **Class**: `general` | **XML ID**: `STSMAX_LDSMAX`

**Architecture**: `FEAT_LSE` (ARMv8.1)

**Summary**: Atomic signed maximum on word or doubleword in memory, without return
...
### Variant: `Integer (STSMAX_LDSMAX_32_memop)` (32-bit no memory ordering)
- **Condition**: `size == 10 && R == 0`
- **Assembly**: `STSMAX  <Ws>, [<Xn|SP>]`
- **Fixed bits**: `size`=`0`, `R`=`0`
- **Bit Pattern**: `??????????????????????0???????0?`
- **Alias of**: `LDSMAX  <Ws>, WZR, [<Xn|SP>]`
...
|-----------------------------------------|
| 1x  111 0   00  0   R   1   Rs  0   100 00  Rn  11111 |
```

### Variant: `Integer (STSMAXL_LDSMAXL_32_memop)` (32-bit release)
- **Condition**: `size == 10 && R == 1`
- **Assembly**: `STSMAXL  <Ws>, [<Xn|SP>]`
- **Fixed bits**: `size`=`0`, `R`=`1`
- **Bit Pattern**: `??????????????????????1???????0?`
- **Alias of**: `LDSMAXL  <Ws>, WZR, [<Xn|SP>]`
  Condition: Unconditionally
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (STSMAX_LDSMAX_64_memop)` (64-bit no memory ordering)
- **Condition**: `size == 11 && R == 0`
- **Assembly**: `STSMAX  <Xs>, [<Xn|SP>]`
- **Fixed bits**: `size`=`1`, `R`=`0`
- **Bit Pattern**: `??????????????????????0???????1?`
- **Alias of**: `LDSMAX  <Xs>, XZR, [<Xn|SP>]`
  Condition: Unconditionally
**Encoding Diagram (32-bit)**:
...
### Variant: `Integer (STSMAXL_LDSMAXL_64_memop)` (64-bit release)
- **Condition**: `size == 11 && R == 1`
- **Assembly**: `STSMAXL  <Xs>, [<Xn|SP>]`
- **Fixed bits**: `size`=`1`, `R`=`1`
- **Bit Pattern**: `??????????????????????1???????1?`
- **Alias of**: `LDSMAXL  <Xs>, XZR, [<Xn|SP>]`
  Condition: Unconditionally
**Encoding Diagram (32-bit)**:
