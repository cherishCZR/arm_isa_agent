...
**Title**: STTRH -- A64 | **Class**: `general` | **XML ID**: `STTRH`

**Summary**: Store register halfword (unprivileged)

**Description**:
This instruction stores a halfword from a 32-bit register
to memory. The address that is used for the store is calculated
...
|--------------------------------------|
| 01  11  1   0   0   0   00  0   imm9 10  Rn  Rt  |
```

#### Decode (A64.ldst.ldst_unpriv.STTRH_32_ldst_unpriv)

```
constant bits(64) offset = SignExtend(imm9, 64);
```

#### Postdecode (A64.ldst.ldst_unpriv.STTRH_32_ldst_unpriv)
...
#### Execute (A64.ldst.ldst_unpriv.STTRH_32_ldst_unpriv)
