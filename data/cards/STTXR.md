...
**Title**: STTXR -- A64 | **Class**: `general` | **XML ID**: `STTXR`

**Architecture**: `FEAT_LSUI` (ARMv9.6)

**Summary**: Store unprivileged exclusive register
...
### Variant: `No offset (STTXR_SR32_ldstexclr_unpriv)` (32-bit)
- **Condition**: `sz == 0`
- **Assembly**: `STTXR  <Ws>, <Wt>, [<Xn|SP>{, #0}]`
- **Fixed bits**: `sz`=`0`
- **Bit Pattern**: `??????????????????????????????0?`
...
|--------------------------------|
| 1   sz  0010010 0   0   Rs  0   (1)(1)(1)(1)(1) Rn  Rt  |
```

#### Decode (A64.ldst.ldstexclr_unpriv.STTXR_SR32_ldstexclr_unpriv)
...
#### Execute (A64.ldst.ldstexclr_unpriv.STTXR_SR32_ldstexclr_unpriv)
...
### Variant: `No offset (STTXR_SR64_ldstexclr_unpriv)` (64-bit)
- **Condition**: `sz == 1`
- **Assembly**: `STTXR  <Ws>, <Xt>, [<Xn|SP>{, #0}]`
- **Fixed bits**: `sz`=`1`
- **Bit Pattern**: `??????????????????????????????1?`
**Encoding Diagram (32-bit)**:
