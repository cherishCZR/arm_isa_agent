...
**Title**: UMAX (immediate) -- A64 | **Class**: `general` | **XML ID**: `UMAX_imm`

**Architecture**: `FEAT_CSSC` (ARMv8.9)

**Summary**: Unsigned maximum (immediate)

**Description**:
This instruction determines the unsigned maximum of the source
register value and immediate, and writes the result to the destination
register.

### Variant: `Integer (UMAX_32U_minmax_imm)` (32-bit)
- **Condition**: `sf == 0`
- **Assembly**: `UMAX  <Wd>, <Wn>, #<uimm>`
- **Fixed bits**: `sf`=`0`
- **Bit Pattern**: `???????????????????????????????0`
...
|--------------------------|
| sf  0   0   1000111 0001 imm8 Rn  Rd  |
```

#### Decode (A64.dpimm.minmax_imm.UMAX_32U_minmax_imm)
...
#### Execute (A64.dpimm.minmax_imm.UMAX_32U_minmax_imm)
...
### Variant: `Integer (UMAX_64U_minmax_imm)` (64-bit)
- **Condition**: `sf == 1`
- **Assembly**: `UMAX  <Xd>, <Xn>, #<uimm>`
- **Fixed bits**: `sf`=`1`
- **Bit Pattern**: `???????????????????????????????1`
**Encoding Diagram (32-bit)**:
