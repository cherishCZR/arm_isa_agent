...
**Title**: UMINV -- A64 | **Class**: `advsimd` | **XML ID**: `UMINV_advsimd`

**Architecture**: `FEAT_AdvSIMD` (ARMv8.0)

**Summary**: Unsigned minimum across vector
...
|-----------------------------------------|
| 0   Q   1   0   111 0   size 11000 1   1010 10  Rn  Rd  |
```

#### Decode (A64.simd_dp.asimdall.UMINV_asimdall_only)
...
#### Execute (A64.simd_dp.asimdall.UMINV_asimdall_only)

```
CheckFPAdvSIMDEnabled64();
constant bits(datasize) operand = V[n, datasize];
integer min = Int(Elem[operand, 0, esize], unsigned);

for e = 1 to elements-1
    constant integer element = Int(Elem[operand, e, esize], unsigned);
    min = Min(min, element);

V[d, esize] = min<esize-1:0>;
```
