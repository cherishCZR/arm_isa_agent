...
**Title**: UMIN (immediate) -- A64 | **Class**: `general` | **XML ID**: `UMIN_imm`

**Architecture**: `FEAT_CSSC` (ARMv8.9)

**Summary**: Unsigned minimum (immediate)

**Description**:
This instruction determines the unsigned minimum of the
source register value and immediate, and writes the result to the destination
register.

### Variant: `Integer (UMIN_32U_minmax_imm)` (32-bit)
- **Condition**: `sf == 0`
- **Assembly**: `UMIN  <Wd>, <Wn>, #<uimm>`
- **Fixed bits**: `sf`=`0`
- **Bit Pattern**: `???????????????????????????????0`
...
|--------------------------|
| sf  0   0   1000111 0011 imm8 Rn  Rd  |
```

#### Decode (A64.dpimm.minmax_imm.UMIN_32U_minmax_imm)
...
#### Execute (A64.dpimm.minmax_imm.UMIN_32U_minmax_imm)

```
constant bits(datasize) operand1 = X[n, datasize];
constant integer result = Min(UInt(operand1), imm);
...
### Variant: `Integer (UMIN_64U_minmax_imm)` (64-bit)
- **Condition**: `sf == 1`
- **Assembly**: `UMIN  <Xd>, <Xn>, #<uimm>`
- **Fixed bits**: `sf`=`1`
- **Bit Pattern**: `???????????????????????????????1`
**Encoding Diagram (32-bit)**:
...
| `<uimm>` | `immediate` | `imm8` | Is an unsigned immediate, in the range 0 to 255, encoded in the "imm8" field. |
