...
**Title**: UMIN (register) -- A64 | **Class**: `general` | **XML ID**: `UMIN_reg`

**Architecture**: `FEAT_CSSC` (ARMv8.9)

**Summary**: Unsigned minimum (register)

**Description**:
This instruction determines the unsigned minimum of the two source
register values and writes the result to the destination register.

### Variant: `Integer (UMIN_32_dp_2src)` (32-bit)
- **Condition**: `sf == 0`
- **Assembly**: `UMIN  <Wd>, <Wn>, <Wm>`
- **Fixed bits**: `sf`=`0`
- **Bit Pattern**: `???????????????????????????????0`
...
|--------------------------|
| sf  0   0   11010110 Rm  011011 Rn  Rd  |
```

#### Decode (A64.dpreg.dp_2src.UMIN_32_dp_2src)
...
#### Execute (A64.dpreg.dp_2src.UMIN_32_dp_2src)

```
constant bits(datasize) operand1 = X[n, datasize];
constant bits(datasize) operand2 = X[m, datasize];
constant integer result = Min(UInt(operand1), UInt(operand2));
...
### Variant: `Integer (UMIN_64_dp_2src)` (64-bit)
- **Condition**: `sf == 1`
- **Assembly**: `UMIN  <Xd>, <Xn>, <Xm>`
- **Fixed bits**: `sf`=`1`
- **Bit Pattern**: `???????????????????????????????1`
**Encoding Diagram (32-bit)**:
