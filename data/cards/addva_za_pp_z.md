...
**Title**: ADDVA -- A64 | **Class**: `mortlach` | **XML ID**: `addva_za_pp_z`

**Architecture**: `FEAT_SME` (PROFILE_A), `FEAT_SME_I16I64` (ARMv9.2)

**Summary**: Add vertically vector elements to ZA tile

**Description**:
This instruction adds each element of the source vector to the corresponding
active element of each vertical slice of a ZA tile.
The tile elements are predicated by a pair of governing predicates.
An element of a vertical slice is considered active if its
corresponding element in the first governing predicate is TRUE
and the element corresponding to its vertical slice number
in the second governing predicate is TRUE.
...
### Variant: `32-bit`
- **Assembly**: `ADDVA  <ZAda>.S, <Pn>/M, <Pm>/M, <Zn>.S`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23 22 21  18  16 15  12   9   4  3  2  1  |
|--------------------------------------------------|
| 1   10  0000 0   1   0   010 00  1   Pm  Pn  Zn  0   0   0   ZAda |
```

#### Decode (A64.sme.mortlach_hvadd.mortlach_addhv.addva_za_pp_z_32)
...
#### Execute (A64.sme.mortlach_hvadd.mortlach_addhv.addva_za_pp_z_32)
...
for row = 0 to dim-1
    constant bits(esize) element = Elem[operand_src, row, esize];
    for col = 0 to dim-1
...
### Variant: `64-bit`
- **Assembly**: `ADDVA  <ZAda>.D, <Pn>/M, <Pm>/M, <Zn>.D`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23 22 21  18  16 15  12   9   4  3  2  |
|-----------------------------------------------|
| 1   10  0000 0   1   1   010 00  1   Pm  Pn  Zn  0   0   ZAda |
```

#### Decode (A64.sme.mortlach_hvadd.mortlach_addhv.addva_za_pp_z_64)
