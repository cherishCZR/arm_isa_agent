...
**Title**: AESEMC -- A64 | **Class**: `sve2` | **XML ID**: `aesemc_mz_zzi`

**Architecture**: `FEAT_SVE_AES2` (ARMv9.6)

**Summary**: Multi-vector AES single round encryption and mix columns

**Description**:
The AESEMC instruction reads a 16-byte state array from each
...
AddRoundKey(), ShiftRows(), SubBytes(), and MixColumns() transformations in accordance with the AES
...
- **Assembly**: `AESEMC  { <Zdn1>.B-<Zdn2>.B }, { <Zdn1>.B-<Zdn2>.B }, <Zm>.Q[<index>]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20  18  16 15  12  10  9   4   0 |
|--------------------------------------------|
| 010 0010 1   00  1   i2  01  1   111 01  0   Zm  Zdn 0   |
```

#### Decode (A64.sve.sve_intx_crypto.sve_crypto_binary_multi2.aesemc_mz_zzi_2x1)
...
#### Execute (A64.sve.sve_intx_crypto.sve_crypto_binary_multi2.aesemc_mz_zzi_2x1)
...
for r = 0 to nreg-1
    ...
    for s = 0 to segments-1
        ...
        Elem[results[r], s, 128] = AESMixColumns(AESSubBytes(AESShiftRows(res)));
...
- **Assembly**: `AESEMC  { <Zdn1>.B-<Zdn4>.B }, { <Zdn1>.B-<Zdn4>.B }, <Zm>.Q[<index>]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20  18  16 15  12  10  9   4   1  |
|--------------------------------------------|
| 010 0010 1   00  1   i2  11  1   111 01  0   Zm  Zdn 00  |
```

#### Decode (A64.sve.sve_intx_crypto.sve_crypto_binary_multi4.aesemc_mz_zzi_4x1)
