...
**Title**: ASRR -- A64 | **Class**: `sve` | **XML ID**: `asrr_z_p_zz`

**Architecture**: `FEAT_SVE || FEAT_SME` (FEAT_SVE || FEAT_SME)

**Summary**: Reversed arithmetic shift right by vector (predicated)

**Description**:
Reversed shift right, preserving the sign bit, active elements of the second source vector by
corresponding elements of the first source vector
and destructively place the results in the corresponding elements of the first source vector. The shift amount operand is a vector of unsigned elements in
which all bits are significant, and not used modulo the
...
### Variant: `SVE`
- **Assembly**: `ASRR  <Zdn>.<T>, <Pg>/M, <Zdn>.<T>, <Zm>.<T>`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20  18 17 16 15  12   9   4  |
|-----------------------------------------|
| 000 0010 0   size 0   10  1   0   0   100 Pg  Zm  Zdn |
```

#### Decode (A64.sve.sve_int_pred_shift.sve_int_bin_pred_shift_1.asrr_z_p_zz_)
...
#### Execute (A64.sve.sve_int_pred_shift.sve_int_bin_pred_shift_1.asrr_z_p_zz_)
...
for e = 0 to elements-1
    if ActivePredicateElement(mask, e, esize) then
        ...
        constant integer shift = Min(UInt(element1), esize);
