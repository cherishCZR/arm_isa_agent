...
**Title**: CPY (immediate, zeroing) -- A64 | **Class**: `sve` | **XML ID**: `cpy_z_o_i`

**Architecture**: `FEAT_SVE || FEAT_SME` (FEAT_SVE || FEAT_SME)

**Summary**: Copy signed integer immediate to vector elements (zeroing)

**Description**:
Copy a signed integer immediate
to each active element in the destination vector.
Inactive elements in the destination vector register are set to zero.
...
### Variant: `SVE`
- **Assembly**: `CPY  <Zd>.<T>, <Pg>/Z, #<imm>{, <shift>}`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21  19  15 14 13 12   4  |
|-----------------------------------|
| 000 0010 1   size 01  Pg  0   0   sh  imm8 Zd  |
```

#### Decode (A64.sve.sve_wideimm_pred.sve_int_dup_imm_pred.cpy_z_o_i_)
...
#### Execute (A64.sve.sve_wideimm_pred.sve_int_dup_imm_pred.cpy_z_o_i_)
