...
**Title**: FAC<cc> -- A64 | **Class**: `sve` | **XML ID**: `facge_p_p_zz`

**Architecture**: `FEAT_SVE || FEAT_SME` (FEAT_SVE || FEAT_SME)

**Summary**: Floating-point absolute compare vectors
...
- **Assembly**: `FACGT  <Pd>.<T>, <Pg>/Z, <Zn>.<T>, <Zm>.<T>`
...
#### Decode (A64.sve.sve_fp_cmpvev.sve_fp_3op_p_pd.facgt_p_p_zz_)
...
#### Execute (A64.sve.sve_fp_cmpvev.sve_fp_3op_p_pd.facgt_p_p_zz_)
...
for e = 0 to elements-1
    if ActivePredicateElement(mask, e, esize) then
        constant bits(esize) element1 = FPAbs(Elem[operand1, e, esize], FPCR);
        constant bits(esize) element2 = FPAbs(Elem[operand2, e, esize], FPCR);
        boolean res;
        case cmp_op of
...
- **Assembly**: `FACGE  <Pd>.<T>, <Pg>/Z, <Zn>.<T>, <Zm>.<T>`
**Encoding Diagram (32-bit)**:
...
#### Decode (A64.sve.sve_fp_cmpvev.sve_fp_3op_p_pd.facge_p_p_zz_)
