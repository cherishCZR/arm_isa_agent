...
**Title**: FAMIN -- A64 | **Class**: `sve2` | **XML ID**: `famin_z_p_zz`

**Architecture**: `(FEAT_SVE2 || FEAT_SME2) && FEAT_FAMINMAX` ((FEAT_SVE2 || FEAT_SME2) && FEAT_FAMINMAX)

**Summary**: Floating-point absolute minimum (predicated)

**Description**:
Determine the minimum  absolute value from active floating-point elements of the second source vector and
 the corresponding floating-point elements of the first source vector
...
### Variant: `SVE2`
- **Assembly**: `FAMIN  <Zdn>.<T>, <Pg>/M, <Zdn>.<T>, <Zm>.<T>`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20 19  15  12   9   4  |
|-----------------------------------|
| 011 0010 1   size 0   0   1111 100 Pg  Zm  Zdn |
```

#### Decode (A64.sve.sve_fp_pred.sve_fp_2op_p_zds.famin_z_p_zz_)
...
#### Execute (A64.sve.sve_fp_pred.sve_fp_2op_p_zds.famin_z_p_zz_)
...
for e = 0 to elements-1
    ...
    if ActivePredicateElement(mask, e, esize) then
        ...
        Elem[result, e, esize] = FPAbsMin(element1, element2, FPCR);
    else
...
| Type | Condition |
|---|---|
| 🔒 FEATURE_GATE | `((IsFeatureImplemented(FEAT_SVE2) \|\| IsFeatureImplemented(FEAT_SME2)) && IsFeatureImplemented(FEAT_FAMINMAX))` |
