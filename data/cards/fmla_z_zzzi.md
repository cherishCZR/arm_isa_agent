...
**Title**: FMLA (indexed) -- A64 | **Class**: `sve` | **XML ID**: `fmla_z_zzzi`

**Architecture**: `FEAT_SVE || FEAT_SME` (FEAT_SVE || FEAT_SME)

**Summary**: Floating-point fused multiply-add by indexed elements (Zda = Zda + Zn * Zm[indexed])

**Description**:
Multiply all floating-point elements within each 128-bit
segment of the first source vector by the specified element in
the corresponding second source vector segment.  The products are then destructively added without intermediate rounding to the corresponding
...
- **Assembly**: `FMLA  <Zda>.H, <Zn>.H, <Zm>.H[<imm>]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23 22 21 20  18  15  11 10  9   4  |
|-----------------------------------------|
| 011 0010 0   0   i3h 1   i3l Zm  0000 0   0   Zn  Zda |
```

#### Decode (A64.sve.sve_fp_fma_by_indexed_elem.sve_fp_fma_by_indexed_elem.fmla_z_zzzi_h)
...
#### Execute (A64.sve.sve_fp_fma_by_indexed_elem.sve_fp_fma_by_indexed_elem.fmla_z_zzzi_h)
...
- **Assembly**: `FMLA  <Zda>.S, <Zn>.S, <Zm>.S[<imm>]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20  18  15  11 10  9   4  |
|--------------------------------------|
| 011 0010 0   10  1   i2  Zm  0000 0   0   Zn  Zda |
```

#### Decode (A64.sve.sve_fp_fma_by_indexed_elem.sve_fp_fma_by_indexed_elem.fmla_z_zzzi_s)
...
- **Assembly**: `FMLA  <Zda>.D, <Zn>.D, <Zm>.D[<imm>]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20 19  15  11 10  9   4  |
|--------------------------------------|
| 011 0010 0   11  1   i1  Zm  0000 0   0   Zn  Zda |
```

#### Decode (A64.sve.sve_fp_fma_by_indexed_elem.sve_fp_fma_by_indexed_elem.fmla_z_zzzi_d)
