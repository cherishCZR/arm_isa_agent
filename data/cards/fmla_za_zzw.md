...
**Title**: FMLA (multiple vectors) -- A64 | **Class**: `mortlach2` | **XML ID**: `fmla_za_zzw`

**Architecture**: `FEAT_SME2` (ARMv9.3), `FEAT_SME_F16F16` (ARMv9.4)

**Summary**: Multi-vector floating-point fused multiply-add

**Description**:
This instruction multiplies the corresponding floating-point elements
of the two or four first and second source vectors and destructively adds these values without intermediate rounding to the corresponding elements of the ZA single-vector groups.
...
- **Assembly**: `FMLA  ZA.<T>[<Wv>, <offs>{, VGx2}], { <Zn1>.<T>-<Zn2>.<T> }, { <Zm1>.<T>-<Zm2>.<T> }`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24  22 21 20  16  14  12   9   5  4  3  2  |
|-----------------------------------------------|
| 1   10  0000 11  sz  1   Zm  00  Rv  110 Zn  0   0   0   off3 |
```

#### Decode (A64.sme.mortlach_multi_array_2a.mortlach_multi2_zz_za_float_mm.fmla_za_zzw_2x2)
...
#### Execute (A64.sme.mortlach_multi_array_2a.mortlach_multi2_zz_za_float_mm.fmla_za_zzw_2x2)
...
- **Assembly**: `FMLA  ZA.H[<Wv>, <offs>{, VGx2}], { <Zn1>.H-<Zn2>.H }, { <Zm1>.H-<Zm2>.H }`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24  22 21 20  16  14  12   9   5  4  3  2  |
|-----------------------------------------------|
| 1   10  0000 11  0   1   Zm  00  Rv  100 Zn  0   0   1   off3 |
```

#### Decode (A64.sme.mortlach_multi_array_2a.mortlach_multi2_zz_za_f16_mm.fmla_za_zzw_2x2_16)
...
- **Assembly**: `FMLA  ZA.<T>[<Wv>, <offs>{, VGx4}], { <Zn1>.<T>-<Zn4>.<T> }, { <Zm1>.<T>-<Zm4>.<T> }`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24  22 21 20  17 16  14  12   9   6   4  3  2  |
|--------------------------------------------------|
| 1   10  0000 11  sz  1   Zm  0   10  Rv  110 Zn  00  0   0   off3 |
```

#### Decode (A64.sme.mortlach_multi_array_2b.mortlach_multi4_zz_za_float_mm.fmla_za_zzw_4x4)
...
- **Assembly**: `FMLA  ZA.H[<Wv>, <offs>{, VGx4}], { <Zn1>.H-<Zn4>.H }, { <Zm1>.H-<Zm4>.H }`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24  22 21 20  17 16  14  12   9   6   4  3  2  |
|--------------------------------------------------|
| 1   10  0000 11  0   1   Zm  0   10  Rv  100 Zn  00  0   1   off3 |
```

#### Decode (A64.sme.mortlach_multi_array_2b.mortlach_multi4_zz_za_f16_mm.fmla_za_zzw_4x4_16)
