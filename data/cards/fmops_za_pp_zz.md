...
**Title**: FMOPS (non-widening) -- A64 | **Class**: `N/A` | **XML ID**: `fmops_za_pp_zz`

**Architecture**: `FEAT_SME_F16F16` (ARMv9.4), `FEAT_SME` (PROFILE_A), `FEAT_SME_F64F64` (ARMv9.2)

**Summary**: Floating-point outer product and subtract
...
- **Assembly**: `FMOPS  <ZAda>.H, <Pn>/M, <Pm>/M, <Zn>.H, <Zm>.H`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23  21 20  15  12   9   4  3   1  0 |
|--------------------------------------------|
| 1   00  0000 1   10  0   Zm  Pm  Pn  Zn  1   10  0   ZAda |
```

#### Decode (A64.sme.mortlach2_misc_prod.mortlach_f16f16_prod.fmops_za_pp_zz_16)
...
#### Execute (A64.sme.mortlach2_misc_prod.mortlach_f16f16_prod.fmops_za_pp_zz_16)
...
for row = 0 to dim-1
    for col = 0 to dim-1
        ...
        if (ActivePredicateElement(mask1, row, esize) &&
            ...
            constant bits(esize) elem1 = FPNeg(Elem[op1, row, esize], FPCR);
...
- **Assembly**: `FMOPS  <ZAda>.S, <Pn>/M, <Pm>/M, <Zn>.S, <Zm>.S`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23  21 20  15  12   9   4  3   1  |
|-----------------------------------------|
| 1   00  0000 0   10  0   Zm  Pm  Pn  Zn  1   00  ZAda |
```

#### Decode (A64.sme.mortlach_32bit_fp_prod.mortlach_f32f32_prod.fmops_za_pp_zz_32)
...
- **Assembly**: `FMOPS  <ZAda>.D, <Pn>/M, <Pm>/M, <Zn>.D, <Zm>.D`
**Encoding Diagram (32-bit)**:

```text
| 31 30 29 28  24 23  21 20  15  12   9   4  3  2  |
|--------------------------------------------|
| 1   0   0   0000 0   11  0   Zm  Pm  Pn  Zn  1   0   ZAda |
```

#### Decode (A64.sme.mortlach_64bit_prod.mortlach_f64f64_prod.fmops_za_pp_zz_64)
