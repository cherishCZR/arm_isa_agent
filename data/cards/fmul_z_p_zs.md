...
**Title**: FMUL (immediate) -- A64 | **Class**: `sve` | **XML ID**: `fmul_z_p_zs`

**Architecture**: `FEAT_SVE || FEAT_SME` (FEAT_SVE || FEAT_SME)

**Summary**: Floating-point multiply by immediate (predicated)

**Description**:
Multiply by an immediate each active floating-point element of the source vector,
and destructively place the results in the corresponding elements of the  source vector.
The immediate may take the value +0.5 or +2.0 only.
Inactive elements in the destination vector register remain unmodified.

**Attributes**: Predicated

### Variant: `SVE`
- **Assembly**: `FMUL  <Zdn>.<T>, <Pg>/M, <Zdn>.<T>, <const>`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20  18  15  12   9   5  4  |
|--------------------------------------|
| 011 0010 1   size 0   11  010 100 Pg  0000 i1  Zdn |
```

#### Decode (A64.sve.sve_fp_pred.sve_fp_2op_i_p_zds.fmul_z_p_zs_)
...
constant bits(esize) imm = if i1 == '0' then FPPointFive('0', esize) else FPTwo('0', esize);
```

#### Execute (A64.sve.sve_fp_pred.sve_fp_2op_i_p_zds.fmul_z_p_zs_)
