...
**Title**: FNMSB -- A64 | **Class**: `sve` | **XML ID**: `fnmsb_z_p_zzz`

**Architecture**: `FEAT_SVE || FEAT_SME` (FEAT_SVE || FEAT_SME)

**Summary**: Floating-point negated fused multiply-subtract vectors (predicated), writing multiplicand [Zdn = -Za + Zdn * Zm]

**Description**:
Multiply the corresponding active floating-point elements of the first and second source
vectors and subtract from elements of the third (addend)
vector without intermediate rounding.
Destructively place the negated results in the destination
and first source (multiplicand) vector.  Inactive elements in the destination vector register remain unmodified.

**Attributes**: Predicated

### Variant: `SVE`
- **Assembly**: `FNMSB  <Zdn>.<T>, <Pg>/M, <Zm>.<T>, <Za>.<T>`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20  15 14 13 12   9   4  |
|--------------------------------------|
| 011 0010 1   size 1   Za  1   1   1   Pg  Zm  Zdn |
```

#### Decode (A64.sve.sve_fp_fma.sve_fp_3op_p_zds_b.fnmsb_z_p_zzz_)
...
constant boolean op3_neg = TRUE;
```

#### Execute (A64.sve.sve_fp_fma.sve_fp_3op_p_zds_b.fnmsb_z_p_zzz_)
