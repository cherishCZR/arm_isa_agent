...
**Title**: FTMOPA (widening, 2-way, FP8 to FP16) -- A64 | **Class**: `mortlach2` | **XML ID**: `ftmopa_za16_z8z8zi`

**Architecture**: `FEAT_SME_TMOP && FEAT_SME_F8F16` (FEAT_SME_TMOP && FEAT_SME_F8F16)

**Summary**: 8-bit floating-point sparse sum of two outer products, accumulating
...
The sum of outer products is generated by multiplying the selected 2-in-4 8-bit floating-point
values from each overlapping 16-bit containers of the two SVLH×2 sub-matrices in
the first source vectors by the two 8-bit floating point
values from the corresponding 16-bit container of
the 2×SVLH sub-matrix in the second source vector.
...
rows of two SVLH×2 sub-matrices. Each 16-bit container of the
second source vector holds 2 elements of columns of a compressed
2×SVLH sub-matrix.

The 2-in-4 8-bit floating values from overlapping 16-bit
...
to half-precision values. The resulting SVLH×SVLH
half-precision sum of outer products is scaled by 2-UInt(FPMR.LSCALE[3:0]), before being destructively
...
### Variant: `SME2`
- **Assembly**: `FTMOPA  <ZAda>.H, { <Zn1>.B-<Zn2>.B }, <Zm>.B, <Zk>[<index>]`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23  21 20  15 14 13 12 11   9   5   3  2  1  0 |
|--------------------------------------------------------|
| 1   00  0000 0   01  1   Zm  0   0   0   K   Zk  Zn  i2  1   0   0   ZAda |
```

#### Decode (A64.sme.mortlach2_ss_prod.mortlach_f8f16_2in4ss_prod.ftmopa_za16_z8z8zi_b2x1)
...
#### Execute (A64.sme.mortlach2_ss_prod.mortlach_f8f16_2in4ss_prod.ftmopa_za16_z8z8zi_b2x1)
...
constant bits(dim*dim*16) op4 = ZAtile[da, 16, dim*dim*16];
        ...
        for r = 0 to 1
            ...
            for e = 0 to 1
                if i < 2 && Elem[ctrl, 4*col + 2*r + e, 1] == '1' then
                    Elem[rowop, i, 8] = Elem[op1, 2*row + e, 8];
                    i = i + 1;
        for j = 0 to 1
            Elem[colop, j, 8] = Elem[op2, 2*col + j, 8];
        constant bits(16) sum = Elem[op4, row*dim+col, 16];
        Elem[result, row*dim+col, 16] = FP8DotAddFP(sum, rowop, colop, FPCR, FPMR);
...
| Type | Condition |
|---|---|
| 🔒 FEATURE_GATE | `IsFeatureImplemented(FEAT_SME_TMOP) && IsFeatureImplemented(FEAT_SME_F8F16)` |
