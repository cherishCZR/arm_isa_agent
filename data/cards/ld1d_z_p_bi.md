...
**Title**: LD1D (scalar plus immediate, single register) -- A64 | **Class**: `N/A` | **XML ID**: `ld1d_z_p_bi`

**Architecture**: `FEAT_SVE || FEAT_SME` (FEAT_SVE || FEAT_SME), `FEAT_SVE2p1` (ARMv9.4)

**Summary**: Contiguous load unsigned doublewords to vector (immediate index)

**Description**:
Contiguous load
   of unsigned doublewords to elements of a vector register from the memory address generated by a 64-bit scalar base and immediate index in the range -8 to 7 which is multiplied by the vector's in-memory size,
...
- **Assembly**: `LD1D  { <Zt>.D }, <Pg>/Z, [<Xn|SP>{, #<imm>, MUL VL}]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  20 19  15  12   9   4  |
|-----------------------------|
| 101 0010 1111 0   imm4 101 Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_memcld.sve_mem_cld_si.ld1d_z_p_bi_u64)
...
#### Execute (A64.sve.sve_memcld.sve_mem_cld_si.ld1d_z_p_bi_u64)
...
- **Assembly**: `LD1D  { <Zt>.Q }, <Pg>/Z, [<Xn|SP>{, #<imm>, MUL VL}]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  22  20 19  15  12   9   4  |
|--------------------------------|
| 101 0010 11  00  1   imm4 001 Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_memcld.sve_mem_cld_si_q.ld1d_z_p_bi_u128)
