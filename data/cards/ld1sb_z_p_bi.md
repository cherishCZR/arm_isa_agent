...
**Title**: LD1SB (scalar plus immediate) -- A64 | **Class**: `sve` | **XML ID**: `ld1sb_z_p_bi`

**Architecture**: `FEAT_SVE || FEAT_SME` (FEAT_SVE || FEAT_SME)

**Summary**: Contiguous load signed bytes to vector (immediate index)

**Description**:
Contiguous load
   of signed bytes to elements of a vector register from the memory address generated by a 64-bit scalar base and immediate index in the range -8 to 7 which is multiplied by the vector's in-memory size,
...
- **Assembly**: `LD1SB  { <Zt>.H }, <Pg>/Z, [<Xn|SP>{, #<imm>, MUL VL}]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  20 19  15  12   9   4  |
|-----------------------------|
| 101 0010 1110 0   imm4 101 Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_memcld.sve_mem_cld_si.ld1sb_z_p_bi_s16)
...
#### Execute (A64.sve.sve_memcld.sve_mem_cld_si.ld1sb_z_p_bi_s16)
...
- **Assembly**: `LD1SB  { <Zt>.S }, <Pg>/Z, [<Xn|SP>{, #<imm>, MUL VL}]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  20 19  15  12   9   4  |
|-----------------------------|
| 101 0010 1101 0   imm4 101 Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_memcld.sve_mem_cld_si.ld1sb_z_p_bi_s32)
...
- **Assembly**: `LD1SB  { <Zt>.D }, <Pg>/Z, [<Xn|SP>{, #<imm>, MUL VL}]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  20 19  15  12   9   4  |
|-----------------------------|
| 101 0010 1100 0   imm4 101 Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_memcld.sve_mem_cld_si.ld1sb_z_p_bi_s64)
