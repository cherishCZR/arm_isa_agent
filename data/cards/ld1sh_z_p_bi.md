...
**Title**: LD1SH (scalar plus immediate) -- A64 | **Class**: `sve` | **XML ID**: `ld1sh_z_p_bi`

**Architecture**: `FEAT_SVE || FEAT_SME` (FEAT_SVE || FEAT_SME)

**Summary**: Contiguous load signed halfwords to vector (immediate index)

**Description**:
Contiguous load
   of signed halfwords to elements of a vector register from the memory address generated by a 64-bit scalar base and immediate index in the range -8 to 7 which is multiplied by the vector's in-memory size,
...
- **Assembly**: `LD1SH  { <Zt>.S }, <Pg>/Z, [<Xn|SP>{, #<imm>, MUL VL}]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  20 19  15  12   9   4  |
|-----------------------------|
| 101 0010 1001 0   imm4 101 Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_memcld.sve_mem_cld_si.ld1sh_z_p_bi_s32)
...
#### Execute (A64.sve.sve_memcld.sve_mem_cld_si.ld1sh_z_p_bi_s32)
...
- **Assembly**: `LD1SH  { <Zt>.D }, <Pg>/Z, [<Xn|SP>{, #<imm>, MUL VL}]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  20 19  15  12   9   4  |
|-----------------------------|
| 101 0010 1000 0   imm4 101 Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_memcld.sve_mem_cld_si.ld1sh_z_p_bi_s64)
