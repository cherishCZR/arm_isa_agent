...
**Title**: LDFF1D (scalar plus vector) -- A64 | **Class**: `sve` | **XML ID**: `ldff1d_z_p_bz`

**Architecture**: `FEAT_SVE` (PROFILE_A)

**Summary**: Gather load first-fault doublewords to vector (vector index)

**Description**:
Gather load with first-faulting behavior
of  doublewords to active elements of a vector register
from memory addresses generated by a
64-bit scalar base plus vector index.
The index values are  optionally  first      sign or zero-extended from 32 to 64 bits and then optionally multiplied by 8.
...
- **Assembly**: `LDFF1D  { <Zt>.D }, <Pg>/Z, [<Xn|SP>, <Zm>.D, <mod> #3]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  22 21 20  15 14 13 12   9   4  |
|--------------------------------------|
| 110 0010 11  xs  1   Zm  0   1   1   Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_mem64.sve_mem_64b_gld_sv.ldff1d_z_p_bz_d_x32_scaled)
...
#### Execute (A64.sve.sve_mem64.sve_mem_64b_gld_sv.ldff1d_z_p_bz_d_x32_scaled)
...
- **Assembly**: `LDFF1D  { <Zt>.D }, <Pg>/Z, [<Xn|SP>, <Zm>.D, <mod>]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  22 21 20  15 14 13 12   9   4  |
|--------------------------------------|
| 110 0010 11  xs  0   Zm  0   1   1   Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_mem64.sve_mem_64b_gld_vs.ldff1d_z_p_bz_d_x32_unscaled)
...
- **Assembly**: `LDFF1D  { <Zt>.D }, <Pg>/Z, [<Xn|SP>, <Zm>.D, LSL #3]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  22  20  15 14 13 12   9   4  |
|-----------------------------------|
| 110 0010 11  11  Zm  1   1   1   Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_mem64.sve_mem_64b_gld_sv2.ldff1d_z_p_bz_d_64_scaled)
...
- **Assembly**: `LDFF1D  { <Zt>.D }, <Pg>/Z, [<Xn|SP>, <Zm>.D]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  22  20  15 14 13 12   9   4  |
|-----------------------------------|
| 110 0010 11  10  Zm  1   1   1   Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_mem64.sve_mem_64b_gld_vs2.ldff1d_z_p_bz_d_64_unscaled)
