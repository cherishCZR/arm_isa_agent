...
**Title**: LDNF1SB -- A64 | **Class**: `sve` | **XML ID**: `ldnf1sb_z_p_bi`

**Architecture**: `FEAT_SVE` (PROFILE_A)

**Summary**: Contiguous load non-fault signed bytes to vector (immediate index)

**Description**:
Contiguous load
 with non-faulting behavior  of signed bytes to elements of a vector register from the memory address generated by a 64-bit scalar base and immediate index in the range -8 to 7 which is multiplied by the vector's in-memory size,
...
- **Assembly**: `LDNF1SB  { <Zt>.H }, <Pg>/Z, [<Xn|SP>{, #<imm>, MUL VL}]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  20 19  15  12   9   4  |
|-----------------------------|
| 101 0010 1110 1   imm4 101 Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_memcld.sve_mem_cldnf_si.ldnf1sb_z_p_bi_s16)
...
#### Execute (A64.sve.sve_memcld.sve_mem_cldnf_si.ldnf1sb_z_p_bi_s16)
...
- **Assembly**: `LDNF1SB  { <Zt>.S }, <Pg>/Z, [<Xn|SP>{, #<imm>, MUL VL}]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  20 19  15  12   9   4  |
|-----------------------------|
| 101 0010 1101 1   imm4 101 Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_memcld.sve_mem_cldnf_si.ldnf1sb_z_p_bi_s32)
...
- **Assembly**: `LDNF1SB  { <Zt>.D }, <Pg>/Z, [<Xn|SP>{, #<imm>, MUL VL}]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  20 19  15  12   9   4  |
|-----------------------------|
| 101 0010 1100 1   imm4 101 Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_memcld.sve_mem_cldnf_si.ldnf1sb_z_p_bi_s64)
