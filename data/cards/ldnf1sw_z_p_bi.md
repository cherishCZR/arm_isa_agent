...
**Title**: LDNF1SW -- A64 | **Class**: `sve` | **XML ID**: `ldnf1sw_z_p_bi`

**Architecture**: `FEAT_SVE` (PROFILE_A)

**Summary**: Contiguous load non-fault signed words to vector (immediate index)

**Description**:
Contiguous load
 with non-faulting behavior  of signed words to elements of a vector register from the memory address generated by a 64-bit scalar base and immediate index in the range -8 to 7 which is multiplied by the vector's in-memory size,
...
### Variant: `SVE`
- **Assembly**: `LDNF1SW  { <Zt>.D }, <Pg>/Z, [<Xn|SP>{, #<imm>, MUL VL}]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  20 19  15  12   9   4  |
|-----------------------------|
| 101 0010 0100 1   imm4 101 Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_memcld.sve_mem_cldnf_si.ldnf1sw_z_p_bi_s64)
...
#### Execute (A64.sve.sve_memcld.sve_mem_cldnf_si.ldnf1sw_z_p_bi_s64)
