...
**Title**: LDNF1W -- A64 | **Class**: `sve` | **XML ID**: `ldnf1w_z_p_bi`

**Architecture**: `FEAT_SVE` (PROFILE_A)

**Summary**: Contiguous load non-fault unsigned words to vector (immediate index)

**Description**:
Contiguous load
 with non-faulting behavior  of unsigned words to elements of a vector register from the memory address generated by a 64-bit scalar base and immediate index in the range -8 to 7 which is multiplied by the vector's in-memory size,
...
- **Assembly**: `LDNF1W  { <Zt>.S }, <Pg>/Z, [<Xn|SP>{, #<imm>, MUL VL}]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  20 19  15  12   9   4  |
|-----------------------------|
| 101 0010 1010 1   imm4 101 Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_memcld.sve_mem_cldnf_si.ldnf1w_z_p_bi_u32)
...
#### Execute (A64.sve.sve_memcld.sve_mem_cldnf_si.ldnf1w_z_p_bi_u32)
...
- **Assembly**: `LDNF1W  { <Zt>.D }, <Pg>/Z, [<Xn|SP>{, #<imm>, MUL VL}]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  20 19  15  12   9   4  |
|-----------------------------|
| 101 0010 1011 1   imm4 101 Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_memcld.sve_mem_cldnf_si.ldnf1w_z_p_bi_u64)
