...
**Title**: LDNT1B (scalar plus immediate, single register) -- A64 | **Class**: `sve` | **XML ID**: `ldnt1b_z_p_bi`

**Architecture**: `FEAT_SVE || FEAT_SME` (FEAT_SVE || FEAT_SME)

**Summary**: Contiguous load non-temporal bytes to vector (immediate index)

**Description**:
Contiguous load
  non-temporal of  bytes to elements of a vector register from the memory address generated by a 64-bit scalar base and immediate index in the range -8 to 7 which is multiplied by the vector's in-memory size,
...
### Variant: `SVE`
- **Assembly**: `LDNT1B  { <Zt>.B }, <Pg>/Z, [<Xn|SP>{, #<imm>, MUL VL}]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  22  20 19  15  12   9   4  |
|--------------------------------|
| 101 0010 00  00  0   imm4 111 Pg  Rn  Zt  |
```

#### Decode (A64.sve.sve_memcld.sve_mem_cldnt_si.ldnt1b_z_p_bi_contiguous)
...
#### Execute (A64.sve.sve_memcld.sve_mem_cldnt_si.ldnt1b_z_p_bi_contiguous)
