## PRFB
_ARM A64 Instruction_

**Title**: PRFB (scalar plus scalar) -- A64 | **Class**: `sve` | **XML ID**: `prfb_i_p_br`

**Architecture**: `FEAT_SVE || FEAT_SME` (FEAT_SVE || FEAT_SME)

**Summary**: Contiguous prefetch bytes (scalar index)

**Description**:
Contiguous prefetch of byte elements
from the memory address generated by a 64-bit scalar base and scalar index
which is
     added to the base address.
After each
    element prefetch
    the index value is incremented,
    but the index register is not updated.

**Attributes**: Predicated

### Variant: `SVE`
- **Assembly**: `PRFB  <prfop>, <Pg>, [<Xn|SP>, <Xm>]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  22  20  15  12   9   4  3  |
|--------------------------------|
| 100 0010 00  00  ?   110 Pg  Rn  0   prfop |
```

#### Decode (A64.sve.sve_mem32.sve_mem_prfm_ss.prfb_i_p_br_s)

```
if !IsFeatureImplemented(FEAT_SVE) && !IsFeatureImplemented(FEAT_SME) then
    EndOfDecode(Decode_UNDEF);
if Rm == '11111' then EndOfDecode(Decode_UNDEF);
constant integer esize = 8;
constant integer g = UInt(Pg);
constant integer n = UInt(Rn);
constant integer m = UInt(Rm);
constant integer level = UInt(prfop<2:1>);
constant boolean stream = (prfop<0> == '1');
constant PrefetchHint pref_hint = if prfop<3> == '0' then Prefetch_READ else Prefetch_WRITE;
constant integer scale = 0;
```

#### Execute (A64.sve.sve_mem32.sve_mem_prfm_ss.prfb_i_p_br_s)

```
CheckSVEEnabled();
constant integer VL = CurrentVL;
constant integer PL = VL DIV 8;
constant integer elements = VL DIV esize;
constant bits(PL) mask = P[g, PL];
bits(64) base;
bits(64) offset;

if AnyActiveElement(mask, esize) then
    base = if n == 31 then SP[64] else X[n, 64];
    offset = X[m, 64];

for e = 0 to elements-1
    if ActivePredicateElement(mask, e, esize) then
        constant integer eoff = UInt(offset) + e;
        constant bits(64) addr = base + (eoff << scale);
        Hint_Prefetch(addr, pref_hint, level, stream);
```

#### Constraints
_1× 🚫 ENCODING_UNDEF / 1× 🔒 FEATURE_GATE_

| Type | Condition |
|---|---|
| 🔒 FEATURE_GATE | `IsFeatureImplemented(FEAT_SVE) \|\| IsFeatureImplemented(FEAT_SME)` |
| 🚫 ENCODING_UNDEF | `Rm != '11111'` |

### Operands

| Symbol | Type | Field | Description |
|---|---|---|---|
| `<prfop>` | `unknown` | `prfop` | Is the prefetch operation specifier, |
| `<Pg>` | `unknown` | `Pg` | Is the name of the governing scalable predicate register P0-P7, encoded in the "Pg" field. |
| `<Xn\|SP>` | `register (64-bit)` | `Rn` | Is the 64-bit name of the general-purpose base register or stack pointer, encoded in the "Rn" field. |
| `<Xm>` | `register (64-bit)` | `Rm` | Is the 64-bit name of the general-purpose offset register, encoded in the "Rm" field. |

**<prfop> Value Table**:

| bitfield | symbol |
|---|---|
| 0000 | PLDL1KEEP |
| 0001 | PLDL1STRM |
| 0010 | PLDL2KEEP |
| 0011 | PLDL2STRM |
| 0100 | PLDL3KEEP |
| 0101 | PLDL3STRM |
| x11x | #uimm4 |
| 1000 | PSTL1KEEP |
| 1001 | PSTL1STRM |
| 1010 | PSTL2KEEP |
| 1011 | PSTL2STRM |
| 1100 | PSTL3KEEP |
| 1101 | PSTL3STRM |

---
<details><summary>Metadata</summary>

- isa: `A64`
- sve-esize: `esize-byte`
- source: `prfb_i_p_br.xml`
</details>