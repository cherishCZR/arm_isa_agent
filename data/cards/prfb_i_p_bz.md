## PRFB
_ARM A64 Instruction_

**Title**: PRFB (scalar plus vector) -- A64 | **Class**: `sve` | **XML ID**: `prfb_i_p_bz`

**Architecture**: `FEAT_SVE` (PROFILE_A)

**Summary**: Gather prefetch bytes (scalar plus vector)

**Description**:
Gather prefetch of  bytes from the active memory addresses generated by a
64-bit scalar base plus vector index.
The index values are  optionally        sign or zero-extended from 32 to 64 bits.
Inactive addresses are not prefetched from memory.

The <prfop> symbol specifies the prefetch hint as a combination of three
options: access type PLD for load or PST for store;
target cache level L1, L2 or L3;
temporality (KEEP for temporal or STRM for non-temporal).

This instruction is illegal when executed in Streaming SVE mode, unless FEAT_SME_FA64 is implemented and enabled.

**Attributes**: Predicated; SM Policy: `SM_0_only`

### Variant: `32-bit scaled offset`
- **Assembly**: `PRFB  <prfop>, <Pg>, [<Xn|SP>, <Zm>.S, <mod>]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  22 21 20  15 14  12   9   4  3  |
|--------------------------------------|
| 100 0010 00  xs  1   Zm  0   00  Pg  Rn  0   prfop |
```

#### Decode (A64.sve.sve_mem32.sve_mem_32b_prfm_sv.prfb_i_p_bz_s_x32_scaled)

```
if !IsFeatureImplemented(FEAT_SVE) then EndOfDecode(Decode_UNDEF);
constant integer esize = 32;
constant integer g = UInt(Pg);
constant integer n = UInt(Rn);
constant integer m = UInt(Zm);
constant integer level = UInt(prfop<2:1>);
constant boolean stream = (prfop<0> == '1');
constant PrefetchHint pref_hint = if prfop<3> == '0' then Prefetch_READ else Prefetch_WRITE;
constant integer offs_size = 32;
constant boolean offs_unsigned = (xs == '0');
constant integer scale = 0;
```

#### Execute (A64.sve.sve_mem32.sve_mem_32b_prfm_sv.prfb_i_p_bz_s_x32_scaled)

```
CheckNonStreamingSVEEnabled();
constant integer VL = CurrentVL;
constant integer PL = VL DIV 8;
constant integer elements = VL DIV esize;
constant bits(PL) mask = P[g, PL];
bits(64) base;
bits(VL) offset;

if AnyActiveElement(mask, esize) then
    base = if n == 31 then SP[64] else X[n, 64];
    offset = Z[m, VL];

for e = 0 to elements-1
    if ActivePredicateElement(mask, e, esize) then
        constant integer off = Int(Elem[offset, e, esize]<offs_size-1:0>, offs_unsigned);
        constant bits(64) addr = base + (off << scale);
        Hint_Prefetch(addr, pref_hint, level, stream);
```

### Variant: `32-bit unpacked scaled offset`
- **Assembly**: `PRFB  <prfop>, <Pg>, [<Xn|SP>, <Zm>.D, <mod>]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  22 21 20  15 14  12   9   4  3  |
|--------------------------------------|
| 110 0010 00  xs  1   Zm  0   00  Pg  Rn  0   prfop |
```

#### Decode (A64.sve.sve_mem64.sve_mem_64b_prfm_sv.prfb_i_p_bz_d_x32_scaled)

```
if !IsFeatureImplemented(FEAT_SVE) then EndOfDecode(Decode_UNDEF);
constant integer esize = 64;
constant integer g = UInt(Pg);
constant integer n = UInt(Rn);
constant integer m = UInt(Zm);
constant integer level = UInt(prfop<2:1>);
constant boolean stream = (prfop<0> == '1');
constant PrefetchHint pref_hint = if prfop<3> == '0' then Prefetch_READ else Prefetch_WRITE;
constant integer offs_size = 32;
constant boolean offs_unsigned = (xs == '0');
constant integer scale = 0;
```

### Variant: `64-bit scaled offset`
- **Assembly**: `PRFB  <prfop>, <Pg>, [<Xn|SP>, <Zm>.D]`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24  22  20  15 14  12   9   4  3  |
|-----------------------------------|
| 110 0010 00  11  Zm  1   00  Pg  Rn  0   prfop |
```

#### Decode (A64.sve.sve_mem64.sve_mem_64b_prfm_sv2.prfb_i_p_bz_d_64_scaled)

```
if !IsFeatureImplemented(FEAT_SVE) then EndOfDecode(Decode_UNDEF);
constant integer esize = 64;
constant integer g = UInt(Pg);
constant integer n = UInt(Rn);
constant integer m = UInt(Zm);
constant integer level = UInt(prfop<2:1>);
constant boolean stream = (prfop<0> == '1');
constant PrefetchHint pref_hint = if prfop<3> == '0' then Prefetch_READ else Prefetch_WRITE;
constant integer offs_size = 64;
constant boolean offs_unsigned = TRUE;
constant integer scale = 0;
```

### Operands

| Symbol | Type | Field | Description |
|---|---|---|---|
| `<prfop>` | `unknown` | `prfop` | Is the prefetch operation specifier, |
| `<Pg>` | `unknown` | `Pg` | Is the name of the governing scalable predicate register P0-P7, encoded in the "Pg" field. |
| `<Xn\|SP>` | `register (64-bit)` | `Rn` | Is the 64-bit name of the general-purpose base register or stack pointer, encoded in the "Rn" field. |
| `<Zm>` | `register (128-bit)` | `Zm` | Is the name of the offset scalable vector register, encoded in the "Zm" field. |
| `<mod>` | `unknown` | `xs` | Is the index extend and shift specifier, |

**<prfop> Value Table**:

| bitfield | symbol |
|---|---|
| 0000 | PLDL1KEEP |
| 0001 | PLDL1STRM |
| 0010 | PLDL2KEEP |
| 0011 | PLDL2STRM |
| 0100 | PLDL3KEEP |
| 0101 | PLDL3STRM |
| x11x | #uimm4 |
| 1000 | PSTL1KEEP |
| 1001 | PSTL1STRM |
| 1010 | PSTL2KEEP |
| 1011 | PSTL2STRM |
| 1100 | PSTL3KEEP |
| 1101 | PSTL3STRM |

**<mod> Value Table**:

| bitfield | symbol |
|---|---|
| 0 | UXTW |
| 1 | SXTW |

### Encoding Constraints
_1× 🔒 FEATURE_GATE_

| Type | Condition |
|---|---|
| 🔒 FEATURE_GATE | `IsFeatureImplemented(FEAT_SVE)` |

---
<details><summary>Metadata</summary>

- isa: `A64`
- sve-esize: `esize-byte`
- source: `prfb_i_p_bz.xml`
</details>