...
**Title**: SDIVR -- A64 | **Class**: `sve` | **XML ID**: `sdivr_z_p_zz`

**Architecture**: `FEAT_SVE || FEAT_SME` (FEAT_SVE || FEAT_SME)

**Summary**: Signed reversed divide (predicated)
...
### Variant: `SVE`
- **Assembly**: `SDIVR  <Zdn>.<T>, <Pg>/M, <Zdn>.<T>, <Zm>.<T>`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20  17 16 15  12   9   4  |
|--------------------------------------|
| 000 0010 0   size 0   101 1   0   000 Pg  Zm  Zdn |
```

#### Decode (A64.sve.sve_int_pred_bin.sve_int_bin_pred_div.sdivr_z_p_zz_)
...
#### Execute (A64.sve.sve_int_pred_bin.sve_int_bin_pred_div.sdivr_z_p_zz_)
...
for e = 0 to elements-1
    if ActivePredicateElement(mask, e, esize) then
        constant integer divisor = SInt(Elem[operand1, e, esize]);
        constant integer dividend = SInt(Elem[operand2, e, esize]);
        integer quotient;
        if divisor == 0 then
            quotient = 0;
