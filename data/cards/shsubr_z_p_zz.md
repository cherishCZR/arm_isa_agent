...
**Title**: SHSUBR -- A64 | **Class**: `sve2` | **XML ID**: `shsubr_z_p_zz`

**Architecture**: `FEAT_SVE2 || FEAT_SME` (FEAT_SVE2 || FEAT_SME)

**Summary**: Signed halving subtract reversed vectors

**Description**:
Subtract active signed elements of the first source vector from
corresponding signed elements of the second source vector, shift
right one bit, and destructively place the results in the
corresponding elements of the first source vector. Inactive elements in the destination vector register remain unmodified.

**Attributes**: Predicated; DIT-sensitive (condition: `FEAT_SVE2 is implemented or FEAT_SME is implemented`)

### Variant: `SVE2`
- **Assembly**: `SHSUBR  <Zdn>.<T>, <Pg>/M, <Zdn>.<T>, <Zm>.<T>`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20  18 17 16 15  13 12   9   4  |
|--------------------------------------------|
| 010 0010 0   size 0   10  1   1   0   10  0   Pg  Zm  Zdn |
```

#### Decode (A64.sve.sve_intx_predicated.sve_intx_pred_arith_binary.shsubr_z_p_zz_)
...
#### Execute (A64.sve.sve_intx_predicated.sve_intx_pred_arith_binary.shsubr_z_p_zz_)
...
for e = 0 to elements-1
    ...
    if ActivePredicateElement(mask, e, esize) then
        constant integer res = (element2 - element1) >> 1;
