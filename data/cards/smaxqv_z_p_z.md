...
**Title**: SMAXQV -- A64 | **Class**: `sve2` | **XML ID**: `smaxqv_z_p_z`

**Architecture**: `FEAT_SVE2p1 || FEAT_SME2p1` (FEAT_SVE2p1 || FEAT_SME2p1)

**Summary**: Signed maximum reduction of quadword vector segments

**Description**:
Signed maximum of the same element numbers from each 128-bit source vector segment,
placing each result into the corresponding element number of the
128-bit SIMD&FP destination register.
Inactive elements in the source vector are treated as the minimum signed integer for the element size.

**Attributes**: Predicated; DIT-sensitive (condition: `FEAT_SVE2 is implemented or FEAT_SME is implemented`)

### Variant: `SVE2`
- **Assembly**: `SMAXQV  <Vd>.<T>, <Pg>, <Zn>.<Tb>`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20  17 16 15  12   9   4  |
|--------------------------------------|
| 000 0010 0   size 0   011 0   0   001 Pg  Zn  Vd  |
```

#### Decode (A64.sve.sve_int_pred_red.sve_int_reduce_1q.smaxqv_z_p_z_)
...
#### Execute (A64.sve.sve_int_pred_red.sve_int_reduce_1q.smaxqv_z_p_z_)
