...
**Title**: SQADD (vectors, predicated) -- A64 | **Class**: `sve2` | **XML ID**: `sqadd_z_p_zz`

**Architecture**: `FEAT_SVE2 || FEAT_SME` (FEAT_SVE2 || FEAT_SME)

**Summary**: Signed saturating addition (predicated)
...
results in the corresponding elements of the first source vector.
...
### Variant: `SVE2`
- **Assembly**: `SQADD  <Zdn>.<T>, <Pg>/M, <Zdn>.<T>, <Zm>.<T>`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20  18 17 16 15  13 12   9   4  |
|--------------------------------------------|
| 010 0010 0   size 0   11  0   0   0   10  0   Pg  Zm  Zdn |
```

#### Decode (A64.sve.sve_intx_predicated.sve_intx_pred_arith_binary_sat.sqadd_z_p_zz_)
...
#### Execute (A64.sve.sve_intx_predicated.sve_intx_pred_arith_binary_sat.sqadd_z_p_zz_)
...
for e = 0 to elements-1
    ...
    if ActivePredicateElement(mask, e, esize) then
        Elem[result, e, esize] = SignedSat(element1 + element2, esize);
    else
