...
**Title**: SQRDCMLAH (vectors) -- A64 | **Class**: `sve2` | **XML ID**: `sqrdcmlah_z_zzz`

**Architecture**: `FEAT_SVE2 || FEAT_SME` (FEAT_SVE2 || FEAT_SME)

**Summary**: Saturating rounding doubling complex integer multiply-add high with rotate
...
### Variant: `SVE2`
- **Assembly**: `SQRDCMLAH  <Zda>.<T>, <Zn>.<T>, <Zm>.<T>, <const>`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20  15 14  12 11   9   4  |
|--------------------------------------|
| 010 0010 0   size 0   Zm  0   01  1   rot Zn  Zda |
```

#### Decode (A64.sve.sve_intx_muladd_unpred.sve_intx_cmla.sqrdcmlah_z_zzz_)
...
#### Execute (A64.sve.sve_intx_muladd_unpred.sve_intx_cmla.sqrdcmlah_z_zzz_)
...
for p = 0 to pairs-1
    ...
    constant integer elt2_a = SInt(Elem[operand2, 2 * p + sel_a, esize]);
    constant integer elt2_b = SInt(Elem[operand2, 2 * p + sel_b, esize]);
