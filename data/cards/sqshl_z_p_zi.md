...
**Title**: SQSHL (immediate) -- A64 | **Class**: `sve2` | **XML ID**: `sqshl_z_p_zi`

**Architecture**: `FEAT_SVE2 || FEAT_SME` (FEAT_SVE2 || FEAT_SME)

**Summary**: Signed saturating shift left by immediate

**Description**:
Shift left by immediate each active signed element of the source vector,
and destructively place the results in the corresponding elements of the
source vector. Each result element is saturated to the
 N-bit element's
signed integer range -2(N-1)  to (2(N-1))-1. The immediate shift amount is an unsigned value in the range 0
to number of bits per element minus 1. Inactive elements in the destination vector register remain unmodified.

**Attributes**: Predicated

### Variant: `SVE2`
- **Assembly**: `SQSHL  <Zdn>.<T>, <Pg>/M, <Zdn>.<T>, #<const>`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20 19  17 16 15  12   9   7   4  |
|--------------------------------------------|
| 000 0010 0   tszh 0   0   01  1   0   100 Pg  tszl imm3 Zdn |
```

#### Decode (A64.sve.sve_int_pred_shift.sve_int_bin_pred_shift_0.sqshl_z_p_zi_)
...
#### Execute (A64.sve.sve_int_pred_shift.sve_int_bin_pred_shift_0.sqshl_z_p_zi_)
