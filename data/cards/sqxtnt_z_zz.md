...
**Title**: SQXTNT -- A64 | **Class**: `sve2` | **XML ID**: `sqxtnt_z_zz`

**Architecture**: `FEAT_SVE2 || FEAT_SME` (FEAT_SVE2 || FEAT_SME)

**Summary**: Signed saturating extract narrow (top)

**Description**:
Saturate the signed integer value in each source element to half the
original source element width, and place the results in the
odd-numbered half-width destination elements, leaving the even-numbered elements
unchanged.

### Variant: `SVE2`
- **Assembly**: `SQXTNT  <Zd>.<T>, <Zn>.<Tb>`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23 22 21 20  18  16 15 14  12 11 10  9   4  |
|--------------------------------------------------|
| 010 0010 1   0   tszh 1   tszl 00  0   0   10  0   0   1   Zn  Zd  |
```

#### Decode (A64.sve.sve_intx_narrowing.sve_intx_extract_narrow.sqxtnt_z_zz_)
...
#### Execute (A64.sve.sve_intx_narrowing.sve_intx_extract_narrow.sqxtnt_z_zz_)
...
for e = 0 to elements-1
    ...
    constant bits(halfesize) res = SignedSat(element1, halfesize);
