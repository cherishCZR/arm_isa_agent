...
**Title**: ST1H (scalar plus scalar, tile slice) -- A64 | **Class**: `mortlach` | **XML ID**: `st1h_za_p_rrr`

**Architecture**: `FEAT_SME` (PROFILE_A)

**Summary**: Contiguous store of halfwords from 16-bit element ZA tile slice

**Description**:
This instruction performs a contiguous store of halfwords from a 16-bit element ZA tile slice. The slice number within the tile is selected by
...
which is multiplied by 2 and added to the base address.
...
### Variant: `SME`
- **Assembly**: `ST1H  { <ZAt><HV>.H[<Ws>, <offs>] }, <Pg>, [<Xn|SP>{, <Xm>, LSL #1}]`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23  21 20  15 14  12   9   4  3  2  |
|--------------------------------------------|
| 1   11  0000 0   01  1   Rm  V   Rs  Pg  Rn  0   ZAt off3 |
```

#### Decode (A64.sme.mortlach_mem.mortlach_contig_store.st1h_za_p_rrr_)
...
#### Execute (A64.sme.mortlach_mem.mortlach_contig_store.st1h_za_p_rrr_)
...
|---|---|---|---|
| `<ZAt>` | `register (128-bit)` | `ZAt` | Is the name of the ZA tile ZA0-ZA1 to be accessed, encoded in the "ZAt" field. |
...
| `<offs>` | `unknown` | `off3` | Is the slice index offset, in the range 0 to 7, encoded in the "off3" field. |
