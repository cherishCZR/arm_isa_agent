...
**Title**: ST1Q -- A64 | **Class**: `mortlach` | **XML ID**: `st1q_za_p_rrr`

**Architecture**: `FEAT_SME` (PROFILE_A)

**Summary**: Contiguous store of quadwords from 128-bit element ZA tile slice

**Description**:
This instruction performs a contiguous store of quadwords from a 128-bit element ZA tile slice. The slice number in the tile is selected by the slice index register,
modulo the number of 128-bit elements in a Streaming SVE vector.
The memory address is generated by scalar base and optional scalar offset
which is multiplied by 16 and added to the base address.
...
### Variant: `SME`
- **Assembly**: `ST1Q  { <ZAt><HV>.Q[<Ws>, <offs>] }, <Pg>, [<Xn|SP>{, <Xm>, LSL #4}]`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24  20  15 14  12   9   4  3  |
|-----------------------------------|
| 1   11  0000 1111 Rm  V   Rs  Pg  Rn  0   ZAt |
```

#### Decode (A64.sme.mortlach_mem.mortlach_contig_qstore.st1q_za_p_rrr_)
...
#### Execute (A64.sme.mortlach_mem.mortlach_contig_qstore.st1q_za_p_rrr_)
...
|---|---|---|---|
| `<ZAt>` | `register (128-bit)` | `ZAt` | Is the name of the ZA tile ZA0-ZA15 to be accessed, encoded in the "ZAt" field. |
...
| `<offs>` | `unknown` | `` | Is the slice index offset, with implicit value 0. |
