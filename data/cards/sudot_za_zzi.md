...
**Title**: SUDOT (4-way, multiple and indexed vector) -- A64 | **Class**: `mortlach2` | **XML ID**: `sudot_za_zzi`

**Architecture**: `FEAT_SME2` (ARMv9.3)

**Summary**: Multi-vector signed by unsigned integer dot-product by indexed elements

**Description**:
This instruction computes the
dot product of four signed 8-bit integer values held in each 32-bit
element of the two or four first source vectors and four unsigned
8-bit integer values in the corresponding indexed 32-bit element of the second source vector.
...
- **Assembly**: `SUDOT  ZA.S[<Wv>, <offs>{, VGx2}], { <Zn1>.B-<Zn2>.B }, <Zm>.B[<index>]`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23  21  19  15 14  12 11   9   5  4  3  2  |
|--------------------------------------------------|
| 1   10  0000 1   01  01  Zm  0   Rv  1   i2  Zn  1   1   1   off3 |
```

#### Decode (A64.sme.mortlach_multi_indexed_2.mortlach_multi2_zza_idx_s.sudot_za_zzi_s2xi)
...
#### Execute (A64.sme.mortlach_multi_indexed_2.mortlach_multi2_zza_idx_s.sudot_za_zzi_s2xi)
...
- **Assembly**: `SUDOT  ZA.S[<Wv>, <offs>{, VGx4}], { <Zn1>.B-<Zn4>.B }, <Zm>.B[<index>]`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23  21  19  15 14  12 11   9   6  5  4  3  2  |
|-----------------------------------------------------|
| 1   10  0000 1   01  01  Zm  1   Rv  1   i2  Zn  0   1   1   1   off3 |
```

#### Decode (A64.sme.mortlach_multi_indexed_3.mortlach_multi4_zza_idx_s.sudot_za_zzi_s4xi)
...
| `<index>` | `unknown` | `i2` | Is the immediate index of a 32-bit group of four 8-bit values within each 128-bit vector segment, in the range 0 to 3, encoded in the "i2" field. |
