...
**Title**: SUMLALL (multiple and indexed vector) -- A64 | **Class**: `mortlach2` | **XML ID**: `sumlall_za_zzi`

**Architecture**: `FEAT_SME2` (ARMv9.3)

**Summary**: Multi-vector signed by unsigned integer multiply-add long-long by indexed element

**Description**:
This instruction
multiplies each signed 8-bit element in the one, two, or four
first source vectors with each unsigned 8-bit indexed element
of the second source vector, widens each product to 32 bits,
...
element index which selects the same element position within each 128-bit vector segment.
The element index range is from 0 to one less than the number of elements per 128-bit segment.
...
- **Assembly**: `SUMLALL  ZA.S[<Wv>, <offs1>:<offs4>], <Zn>.B, <Zm>.B[<index>]`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23  21  19  15 14  12   9   4  3  2  1  |
|-----------------------------------------------|
| 1   10  0000 1   00  00  Zm  i4h Rv  i4l Zn  1   0   1   off2 |
```

#### Decode (A64.sme.mortlach_multi_indexed_1.mortlach_multi1_mla_long_long_idx_s.sumlall_za_zzi_s)
...
#### Execute (A64.sme.mortlach_multi_indexed_1.mortlach_multi1_mla_long_long_idx_s.sumlall_za_zzi_s)
...
- **Assembly**: `SUMLALL  ZA.S[<Wv>, <offs1>:<offs4>{, VGx2}], { <Zn1>.B-<Zn2>.B }, <Zm>.B[<index>]`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23  21  19  15 14  12 11   9   5  4  3  2   0 |
|-----------------------------------------------------|
| 1   10  0000 1   00  01  Zm  0   Rv  0   i4h Zn  1   1   0   i4l o1  |
```

#### Decode (A64.sme.mortlach_multi_indexed_2.mortlach_multi2_mla_long_long_idx_s.sumlall_za_zzi_s2xi)
...
- **Assembly**: `SUMLALL  ZA.S[<Wv>, <offs1>:<offs4>{, VGx4}], { <Zn1>.B-<Zn4>.B }, <Zm>.B[<index>]`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23  21  19  15 14  12 11   9   6  5  4  3  2   0 |
|--------------------------------------------------------|
| 1   10  0000 1   00  01  Zm  1   Rv  0   i4h Zn  0   1   1   0   i4l o1  |
```

#### Decode (A64.sme.mortlach_multi_indexed_3.mortlach_multi4_mla_long_long_idx_s.sumlall_za_zzi_s4xi)
