...
**Title**: UMLALL (multiple and indexed vector) -- A64 | **Class**: `mortlach2` | **XML ID**: `umlall_za_zzi`

**Architecture**: `FEAT_SME2` (ARMv9.3), `FEAT_SME2 && FEAT_SME_I16I64` (FEAT_SME2 && FEAT_SME_I16I64)

**Summary**: Multi-vector unsigned integer multiply-add long-long by indexed element

**Description**:
This instruction
multiplies each unsigned 8-bit or 16-bit element in the one, two, or four
first source vectors with each unsigned 8-bit or 16-bit indexed
element of the second source vector, widens each product to 32 bits or 64 bits,
and destructively adds these values to the corresponding 32-bit
...
- **Assembly**: `UMLALL  ZA.S[<Wv>, <offs1>:<offs4>], <Zn>.B, <Zm>.B[<index>]`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23  21  19  15 14  12   9   4  3  2  1  |
|-----------------------------------------------|
| 1   10  0000 1   00  00  Zm  i4h Rv  i4l Zn  1   0   0   off2 |
```

#### Decode (A64.sme.mortlach_multi_indexed_1.mortlach_multi1_mla_long_long_idx_s.umlall_za_zzi_s)
...
#### Execute (A64.sme.mortlach_multi_indexed_1.mortlach_multi1_mla_long_long_idx_s.umlall_za_zzi_s)
...
for r = 0 to nreg-1
    ...
    for i = 0 to 3
        ...
        for e = 0 to elements-1
            ...
            constant integer element2 = UInt(Elem[operand2, s, esize DIV 4]);
...
- **Assembly**: `UMLALL  ZA.D[<Wv>, <offs1>:<offs4>], <Zn>.H, <Zm>.H[<index>]`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23  21  19  15 14  12 11   9   4  3  2  1  |
|--------------------------------------------------|
| 1   10  0000 1   10  00  Zm  i3h Rv  0   i3l Zn  1   0   0   off2 |
```

#### Decode (A64.sme.mortlach_multi_indexed_1.mortlach_multi1_mla_long_long_idx_d.umlall_za_zzi_d)
...
- **Assembly**: `UMLALL  ZA.S[<Wv>, <offs1>:<offs4>{, VGx2}], { <Zn1>.B-<Zn2>.B }, <Zm>.B[<index>]`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23  21  19  15 14  12 11   9   5  4  3  2   0 |
|-----------------------------------------------------|
| 1   10  0000 1   00  01  Zm  0   Rv  0   i4h Zn  0   1   0   i4l o1  |
```

#### Decode (A64.sme.mortlach_multi_indexed_2.mortlach_multi2_mla_long_long_idx_s.umlall_za_zzi_s2xi)
...
- **Assembly**: `UMLALL  ZA.D[<Wv>, <offs1>:<offs4>{, VGx2}], { <Zn1>.H-<Zn2>.H }, <Zm>.H[<index>]`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23  21  19  15 14  12  10  9   5  4  3  2   0 |
|-----------------------------------------------------|
| 1   10  0000 1   10  01  Zm  0   Rv  00  i3h Zn  0   1   0   i3l o1  |
```

#### Decode (A64.sme.mortlach_multi_indexed_2.mortlach_multi2_mla_long_long_idx_d.umlall_za_zzi_d2xi)
...
- **Assembly**: `UMLALL  ZA.S[<Wv>, <offs1>:<offs4>{, VGx4}], { <Zn1>.B-<Zn4>.B }, <Zm>.B[<index>]`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23  21  19  15 14  12 11   9   6  5  4  3  2   0 |
|--------------------------------------------------------|
| 1   10  0000 1   00  01  Zm  1   Rv  0   i4h Zn  0   0   1   0   i4l o1  |
```

#### Decode (A64.sme.mortlach_multi_indexed_3.mortlach_multi4_mla_long_long_idx_s.umlall_za_zzi_s4xi)
...
- **Assembly**: `UMLALL  ZA.D[<Wv>, <offs1>:<offs4>{, VGx4}], { <Zn1>.H-<Zn4>.H }, <Zm>.H[<index>]`
**Encoding Diagram (32-bit)**:

```text
| 31 30  28  24 23  21  19  15 14  12  10  9   6   4  3  2   0 |
|-----------------------------------------------------|
| 1   10  0000 1   10  01  Zm  1   Rv  00  i3h Zn  00  1   0   i3l o1  |
```

#### Decode (A64.sme.mortlach_multi_indexed_3.mortlach_multi4_mla_long_long_idx_d.umlall_za_zzi_d4xi)
