...
**Title**: UQRSHLR -- A64 | **Class**: `sve2` | **XML ID**: `uqrshlr_z_p_zz`

**Architecture**: `FEAT_SVE2 || FEAT_SME` (FEAT_SVE2 || FEAT_SME)

**Summary**: Unsigned saturating rounding shift left reversed vectors (predicated)

**Description**:
Shift active unsigned elements of the second source vector by
corresponding elements of the first source vector and destructively
place the rounded results in the corresponding elements of the first source
vector.
A positive shift amount performs a left shift, otherwise a right shift by the
negated shift amount is performed. Each result element is saturated to the
 N-bit element's
...
### Variant: `SVE2`
- **Assembly**: `UQRSHLR  <Zdn>.<T>, <Pg>/M, <Zdn>.<T>, <Zm>.<T>`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20 19 18 17 16 15  13 12   9   4  |
|-----------------------------------------------|
| 010 0010 0   size 0   0   1   1   1   1   10  0   Pg  Zm  Zdn |
```

#### Decode (A64.sve.sve_intx_predicated.sve_intx_bin_pred_shift_sat_round.uqrshlr_z_p_zz_)
...
#### Execute (A64.sve.sve_intx_predicated.sve_intx_bin_pred_shift_sat_round.uqrshlr_z_p_zz_)
...
constant bits(VL) operand1 = if AnyActiveElement(mask, esize) then Z[m, VL] else Zeros(VL);
constant bits(VL) operand2 = Z[dn, VL];
bits(VL) result;
