...
**Title**: URSHL -- A64 | **Class**: `sve2` | **XML ID**: `urshl_z_p_zz`

**Architecture**: `FEAT_SVE2 || FEAT_SME` (FEAT_SVE2 || FEAT_SME)

**Summary**: Unsigned rounding shift left by vector (predicated)

**Description**:
Shift active unsigned elements of the first source vector by
corresponding elements of the second source vector and destructively
place the rounded results in the corresponding elements of the first source
vector. A positive shift amount performs a left shift, otherwise a right shift by the
negated shift amount is performed. Inactive elements in the destination vector register remain unmodified.

**Attributes**: Predicated

### Variant: `SVE2`
- **Assembly**: `URSHL  <Zdn>.<T>, <Pg>/M, <Zdn>.<T>, <Zm>.<T>`
**Encoding Diagram (32-bit)**:

```text
| 31  28  24 23  21 20 19 18 17 16 15  13 12   9   4  |
|-----------------------------------------------|
| 010 0010 0   size 0   0   0   0   1   1   10  0   Pg  Zm  Zdn |
```

#### Decode (A64.sve.sve_intx_predicated.sve_intx_bin_pred_shift_sat_round.urshl_z_p_zz_)
...
#### Execute (A64.sve.sve_intx_predicated.sve_intx_bin_pred_shift_sat_round.urshl_z_p_zz_)
